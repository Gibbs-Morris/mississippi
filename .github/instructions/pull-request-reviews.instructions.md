...
- Reviews **MUST** flag when code changes affect behavior documented in `docs/Docusaurus/docs/` but corresponding documentation is not updated. Why: Prevents documentation drift.
- When flagging documentation drift, reviewers **SHOULD** identify the specific documentation files that need updates. Why: Actionable feedback enables faster fixes.
...
- Inspect architecture boundaries, DI/logging patterns, and docs/tests.
- Check if code changes require documentation updates in `docs/Docusaurus/docs/`.
