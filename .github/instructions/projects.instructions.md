...
Governing thought: All `.csproj` files follow minimal, centralized package management standards to ensure consistency and maintainability across .NET applications.

## Rules (RFC 2119)

- Project files **MUST** be as minimal as possible; project files **MUST NOT** duplicate information from `Directory.Build.props`.  
  Why: Avoids redundancy and maintains single source of truth for shared configuration.
- Project files **MUST** use centralized package management via `Directory.Packages.props`.  
  Why: Prevents version conflicts and ensures consistent package versions across solution.
- Project files **MUST NOT** specify version numbers in `PackageReference` elements.  
  Why: All versions are centrally managed; local versions would create conflicts.
- Agents **MUST** remove any version attributes from `PackageReference` elements.  
  Why: Enforces centralized package management policy.
- Project files **MUST** validate with clean builds; agents **MUST** resolve all warnings.  
  Why: Warnings are treated as errors in CI per Build Rules.
- Agents **MUST NOT** override automatic assembly/namespace naming without justification.  
  Why: Automatic naming ensures consistency across the solution.
- Agents **SHOULD** choose the correct SDK (`Microsoft.NET.Sdk` or `.Web`) for the project type.  
  Why: SDK determines available features and build behavior.
- Agents **SHOULD** include only project-specific properties in `.csproj` files.  
  Why: Common properties are inherited automatically from `Directory.Build.props`.

## Scope and Audience

**Audience:** Developers creating or modifying .NET project files (`.csproj`) in the repository.

**In scope:** Project file structure, centralized package management, minimal configuration.

**Out of scope:** Specific NuGet package choices, build script implementation.
...
This document defines project file standards for .NET applications using centralized package management, ensuring consistency and maintainability.

## Core Principles

### Keep Project Files Minimal

- Project files should be as minimal as possible
- Avoid duplicating information already defined in `Directory.Build.props`
- Use centralized package management through `Directory.Packages.props`
- Leverage inherited common properties, settings, and configurations
- Focus on project-specific concerns only
...
#### Benchmark Projects Get Extra Configuration

Projects ending with `Benchmarks` can receive BenchmarkDotNet wiring via `Directory.Build.props` (packages and optional `InternalsVisibleTo` support).

See `.github/instructions/benchmarks.instructions.md` for the project naming and folder layout conventions.
