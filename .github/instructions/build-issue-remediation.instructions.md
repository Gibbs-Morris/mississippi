...
- When deferring, create/update a scratchpad task under `.scratchpad/tasks` with `status=deferred`, a concise `reason`, and `nextSteps` so other agents can pick it up (see `.github/instructions/agent-scratchpad.instructions.md`). The scratchpad is ephemeral and ignored by Git; do not reference it from source or tests.
