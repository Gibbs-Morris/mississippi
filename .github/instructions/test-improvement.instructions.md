...
- For coverage hot spots or other follow-ups that require parallel attention, rerun `summarize-coverage-gaps.ps1` to sync tasks into `.scratchpad/tasks/pending` automatically. Mutation survivors are mirrored the same way by `summarize-mutation-survivors.ps1`. Only create manual scratchpad tasks for gaps not covered by these scripts (see `.github/instructions/agent-scratchpad.instructions.md`). The scratchpad is ephemeral and ignored by Git; never reference it from source or tests.

## 🚨 CRITICAL RULE: ZERO WARNINGS IN TESTS 🚨

### ATTENTION: Zero warnings policy applies to test code too
