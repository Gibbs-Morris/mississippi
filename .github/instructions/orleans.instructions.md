...
- You **MUST NOT** create a `Grains/` folder or other horizontal technical silos (e.g., `Models`, `Dtos`, `Services`, `Repositories`).  
  Why: Horizontal folders scatter grain code away from the business features they implement, reducing cohesion.
- You **MUST** place grains with their feature (e.g., `Mississippi/EventSourcing/Orders/OrderGrain.cs`), not under a generic `Grains/` directory.  
  Why: Co-locating grains with features keeps related code together and mirrors domain structure.
- Namespaces **MUST** mirror folders and follow `Company.Product.Feature[.SubFeature]`.  
  Why: Namespace-folder alignment ensures predictable navigation and import statements.
- Type names **SHOULD** carry the `…Grain` suffix; folders **MUST NOT** use the `Grain` suffix.  
  Why: Type suffixes clarify grain roles without creating redundant folder hierarchies.

## Core Principles

### Folder Layout (Vertical Slices)

Organize grains and their collaborators under the feature they belong to, mirroring the namespace: `Company/Product/Feature[/SubFeature]`.
