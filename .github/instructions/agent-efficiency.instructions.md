...
- Agents **MUST** query the existing operation's status through its handle or another authoritative status source before considering a restart after an observation timeout. Why: Merely finding a handle does not establish whether work has stopped.
- Agents **MUST NOT** restart an operation whose queried status is still running merely because an observation timed out. Why: Restarting active work can waste resources or repeat side effects.
