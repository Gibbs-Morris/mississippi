...
- Sample projects under `samples/**` **MUST** use the namespace root implied by the nearest sample-local `Directory.Build.props`; contributors **MUST NOT** prepend `Mississippi` to sample namespaces unless that sample's own build props do so. Why: Samples are illustrative applications with their own project identity and do not automatically share the framework namespace root.
