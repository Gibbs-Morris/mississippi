...
- `NotificationPulse` **MAY** receive nullable `IsExpanded` and `DetailsId` values with `OnExpand` for a parent-controlled disclosure. A `null` `IsExpanded` omits `aria-expanded`, a `null` `DetailsId` omits `aria-controls`, and callback intent remains one-way regardless of metadata. Why: Optional disclosure metadata extends the native action contract without adding an internal toggle or service.
- The parent **MUST** own expanded state, actual callback availability, a stable unique nonblank per-instance `DetailsId` identifying a controlled region that remains mounted and is hidden while collapsed, and focus after the accepted render. Why: The parent owns the state transition and rendered targets; `NotificationPulse` emits intent and renders supplied metadata but does not resolve DOM IDs, prove uniqueness, or manage focus. The component rejects a supplied blank `DetailsId` when `OnExpand` is present, but does not check DOM existence or uniqueness; its action text validation remains in the component.
