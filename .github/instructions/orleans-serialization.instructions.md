...
- While the repository is pre-1.0 (see `.github/instructions/backwards-compatibility.instructions.md`), member IDs and type shapes **MAY** be changed freely; agents **MUST NOT** add compatibility shims for serialization layouts that only exist on the current branch. Why: Pre-release; no rolling deployments to protect.
- Once the repository reaches 1.0+, existing member IDs **MUST NOT** change; breaking type changes (e.g., record-to-class or signedness changes) **MUST NOT** be made; new members **MUST** use unused IDs. Why: Protects rolling deployments.
