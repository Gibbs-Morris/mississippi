...
- Every governed specialist **MUST** return a structured, metadata-sized status envelope to `cs River Orchestrator` before control returns; governed specialists **MUST NOT** append to `.thinking/<task-folder>/activity-log.md` directly. Why: The redesigned workflow makes `cs River Orchestrator` the sole direct writer of operational log entries while keeping specialist progress observable.
- Governed specialists **MUST** write substantive outputs only to declared `.thinking/<task-folder>/` artifact paths or bundle directories and **MUST** return only concise summary metadata, artifact paths, blockers, and next-action guidance; materially revised delegated outputs **MUST** publish new paths rather than overwrite prior handed-back artifacts in place. Why: File-first handoff keeps payloads small while preserving immutable artifact lineage.
...
- `.thinking/<task-folder>/workflow-audit/` **MUST** be treated as the authoritative execution record, with immutable `meta.json` plus immutable seven-digit sequence files; `sequence` **MUST** remain the only chronology authority, canonical `eventUtc` **MUST** be the timing and diagnostics input, and `activity-log.md`, handover logs, Mermaid output, and PR prose **MUST NOT** override canonical facts. Why: Workflow conformance and reviewer trust depend on one append-only source of truth.
...
- `cs Entrepreneur` **MUST NOT** create governed workflow state, `.thinking/<task-folder>/workflow-audit/`, or any canonical audit fact. Why: The optional pre-governed lane shapes ideas but never starts governed delivery.
...
- Every Phase 9 PR Manager execution slice **MUST** begin with explicit `cs River Orchestrator` delegation that names the bounded task slice and sets `details.expectedOutputPath`, `details.completionSignal`, `details.closureCondition`, `details.allowedActions`, and `details.authorizedTargets` while not transferring canonical ownership; `details.expectedOutputPath` **MUST** name a fresh artifact path or fresh bundle directory, and `cs River Orchestrator` **MUST** validate returned artifact existence and containment before canonical completion. Why: Capability-scoped delegation keeps canonical ownership unambiguous without a Phase 9 handoff boundary.
...
- `cs River Orchestrator` **MUST** treat clear governed intake as end-to-end delivery by default and **MUST NOT** hand back a governed run early when the request remains actionable inside the approved workflow. Why: River is the governed orchestrator, so partial summaries are not a completion state.
- When intake appears partial or ambiguous, `cs River Orchestrator` **MUST** ask exactly one qualification round through the question UI that captures execution scope and discovery mode before discovery continues. Why: The redesign allows one clarification checkpoint without reopening the old multi-round qualification ambiguity.
- When that qualification round is not needed, `cs River Orchestrator` **MUST** default to `execution scope = full-run` and `discovery mode = autonomous-defaults`. Why: Governed intake should bias toward finishing the job rather than waiting for unnecessary scope confirmation.
- In `autonomous-defaults`, `cs River Orchestrator` **MUST** delegate discovery-default generation to `cs Requirements Analyst` and **MUST NOT** self-answer discovery questions. Why: River remains an orchestrator and canonical writer, not a specialist discovery worker.
- In `manual-refinement`, the existing five-question human discovery loop **MUST** remain available and **MUST** still use ranked options plus the default-choice escape hatch. Why: Human-guided discovery remains the fallback when the user wants to refine scope directly.
- Autonomous discovery artifacts **MUST** reuse `01-discovery/questions-round-NN.md` and **MUST** label each inferred answer with trust tier, source category, evidence reference(s), confidence, and `requiresHumanConfirmation`; `requirements-synthesis.md` **MUST** keep inferred defaults separate from confirmed requirements until qualification or G1 accepts them. Why: Provenance-aware defaults only stay trustworthy when inference and confirmation remain visibly distinct.
...
- Clean Squad executable metadata in agent frontmatter **MUST** stay in parity with the rules defined in `.github/clean-squad/WORKFLOW.md`. Why: The workflow is the authoritative contract; agent frontmatter is the executable surface.
- Only `cs Entrepreneur` **MAY** expose a public handoff, and that handoff **MUST** target `cs River Orchestrator` with explicit `send: false`. Why: The public surface stays two-entry, one-way, and G0-safe.
- Internal Clean Squad agents **MUST** keep `user-invocable: false`; explicit coordinator allowlists **MUST** remain the bounded delegation control surface for approved paths. Why: Internal workers stay hidden from direct human invocation while delegation remains deliberate.
- Approved phase coordinators **MUST** use explicit `agents:` allowlists, while non-delegating Clean Squad agents **MUST** use `agents: []`. Why: Delegation intent should be executable instead of implicit.
- Shared procedures that repeat across multiple agents **SHOULD** live in `.github/skills/clean-squad-*/SKILL.md`, and consuming agents **MUST** link those skills explicitly from their bodies. Why: Reuse belongs in skills, while normative authority stays in workflow and agent contracts.
- Prompt files, tool-set templates, and hooks **MUST** be treated as operator ergonomics and validation support only; they **MUST NOT** widen the public authority model or override workflow rules. Why: Convenience layers are subordinate to governance.
- Hook automation **MUST** remain non-destructive by default and **MUST** document disabled-mode behavior. Why: Preview automation should improve safety, not create hidden mandatory behavior.
- Nested or parallel coordinator waves **MUST** follow the deterministic batch contract documented in `WORKFLOW.md`, including immutable input manifests, unique output paths, deterministic fan-in ordering, and one explicit synthesis artifact. Why: Parallelism without deterministic joins creates drift and audit ambiguity.
