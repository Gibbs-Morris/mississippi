...
- `.thinking/<task-folder>/workflow-audit.json` **MUST** be treated as the authoritative execution record; `activity-log.md`, handover logs, Mermaid output, and PR prose **MUST NOT** override canonical sequence facts. Why: Workflow conformance and reviewer trust depend on one append-only source of truth.
- The Product Owner **MUST** append canonical audit events only for Phases 1 through 8, and the PR Manager **MUST** append canonical audit events only for Phase 9; all other Clean Squad agents **MUST NOT** write canonical workflow facts. Why: Canonical writes stay aligned to the orchestration boundaries that actually control the measured workflow.
- Only one canonical writer **MUST** be active for a workflow boundary at a time, and the Product Owner to PR Manager handoff **MUST** be explicit before Phase 9 canonical writes begin. Why: Writer exclusivity and an explicit handoff keep canonical ownership unambiguous across the orchestration boundary.
- Canonical audit appends **MUST** declare the expected prior `sequence` and **MUST** fail closed when the ledger tail does not match. Why: Retry safety and writer handoff integrity depend on explicit append preconditions.
- The Scribe **MUST** compile derived audit artifacts from a stable `workflow-audit.json` snapshot, emit provenance with those artifacts, and **MUST NOT** backfill missing canonical facts from secondary logs. Why: Derived reports are trustworthy only when provenance is explicit and canonical gaps remain visible.
- The PR Manager **MUST** treat the `Reviewer Audit Summary` as stale when the HEAD SHA, required CI-result identity, or reviewer-meaningful canonical facts change, and merge readiness **MUST NOT** pass on stale, missing, or mismatched provenance. Why: Reviewer-facing trust depends on freshness bound to the exact code and checks under review.
