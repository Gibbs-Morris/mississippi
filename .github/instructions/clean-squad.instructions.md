...
- Only one canonical writer **MUST** be active for the workflow run at a time, `currentOwner` **MUST** mean canonical ownership only, and every Phase 9 PR Manager execution slice **MUST** begin with explicit Product Owner delegation that names the bounded task slice, expected artifact output or artifact bundle, completion signal, closure condition, allowedActions, and authorizedTargets while not transferring canonical ownership. Why: Writer exclusivity and capability-scoped delegation keep canonical ownership unambiguous without a Phase 9 handoff boundary.
