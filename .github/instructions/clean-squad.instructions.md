...
- Canonical audit appends **MUST** include canonical `eventUtc`, **MUST** declare the expected prior `sequence`, and **MUST** fail closed when the ledger tail does not match. Why: Retry safety, timing derivation, and writer handoff integrity depend on explicit append preconditions.
- Canonical audit events **MUST** use the v3 semantic envelope defined in `.github/clean-squad/WORKFLOW.md`; meaningful events **MUST** carry `workItemId`, `rootWorkItemId`, `spanId`, `causedBy`, `closes`, `outcome`, `artifactTransitions`, and `provenance` whenever the writer-obligation matrix requires them. Why: Reviewer trust depends on explicit canonical semantics rather than chronology-first inference.
...
- The PR Manager **MUST** treat the `Reviewer Audit Summary` as stale when the HEAD SHA, required CI-result identity, or reviewer-meaningful canonical facts change, **MUST** mark that stale state on the PR surface immediately, and merge readiness **MUST NOT** pass on stale, missing, or mismatched provenance. Why: Reviewer-facing audit output is freshness-verifiable within the repo workflow but not tamper-resistant, so explicit stale publication is required.
