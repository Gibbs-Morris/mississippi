...
- Every Phase 9 PR Manager execution slice **MUST** begin with explicit Product Owner delegation that names the bounded task slice, expected artifact output or artifact bundle, completion signal, closure condition, allowedActions, and authorizedTargets while not transferring canonical ownership. Why: Capability-scoped delegation keeps canonical ownership unambiguous without a Phase 9 handoff boundary.
