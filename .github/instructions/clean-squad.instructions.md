...
- Canonical audit appends **MUST** include canonical `eventUtc`, **MUST** encode the expected prior `sequence` in `appendPrecondition.expectedPriorSequence`, and **MUST** fail closed when the ledger tail does not match. Why: Retry safety, timing derivation, and writer handoff integrity depend on explicit append preconditions.
