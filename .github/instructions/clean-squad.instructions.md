...
- `cs Entrepreneur` **MUST NOT** create governed workflow state, `.thinking/<task>/workflow-audit.json`, or any canonical audit fact; it **MUST** hand off only through one Story Pack candidate or an explicit stop outcome. Why: The optional pre-governed lane shapes ideas but never starts governed delivery.
