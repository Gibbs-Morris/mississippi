...
- Clean Squad executable metadata **MUST** stay in parity with `.github/clean-squad/customization-manifest.json`, and that manifest **MUST** remain subordinate to `WORKFLOW.md` rather than replacing it. Why: The workflow is the contract; the manifest is the drift-detection surface.
...
- Internal Clean Squad agents **MUST** keep `user-invocable: false` and `disable-model-invocation: true`; explicit coordinator allowlists **MAY** override that protection only for approved bounded delegation paths. Why: Protected workers should be deliberate targets, not casual model-selected helpers.
...
- Nested or parallel coordinator waves **MUST** follow the deterministic batch contract documented in `WORKFLOW.md` and `SUBAGENT-CONTRACTS.md`, including immutable input manifests, unique output paths, deterministic fan-in ordering, and one explicit synthesis artifact. Why: Parallelism without deterministic joins creates drift and audit ambiguity.
