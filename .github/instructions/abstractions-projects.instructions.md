...
- **Contracts-Only Scope** — `X.Y[.Feature].Abstractions` projects **MUST** contain only public contracts (interfaces, abstract base types, DTOs, domain exceptions, and CQRS requests) with no infrastructure, DI, persistence, or hosting glue. *Why:* Keeps the dependency surface lightweight and reusable across services. **Note:** Abstract base classes intended for external inheritance (rare) should end with 'Base' suffix (e.g., `CommandHandlerBase`) and include documented justification for public visibility.
