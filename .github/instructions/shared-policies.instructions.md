...
- **SDK Aggregators Stay Empty** - `Sdk.Client`, `Sdk.Gateway`, and `Sdk.Runtime` **MUST** remain code-empty packaging aggregation projects (references/metadata only). Builder logic, DI registration logic, and other production code **MUST NOT** be implemented in SDK projects. Why: SDK packages are single-purpose NuGet entrypoints; implementation belongs in non-SDK projects.
