...
- Do not reference `.scratchpad/` paths from source code, projects, or tests
- Keep files small and line-oriented where practical; avoid large binaries
...
- Automation: use the CLI wrappers under `scripts/tasks/` to keep JSON consistent and file moves atomic:
  ...
  - For orchestration or custom automation, review `scripts/tasks/README.md` plus the Pester suite in `scripts-tests/`.
