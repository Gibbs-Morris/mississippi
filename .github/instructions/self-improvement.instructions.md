...
> Drift check: Review the Rules Manager agent (`rules-manager.agent.md`) for the conflict-detection process before adding lessons; open relevant domain instruction files to check for overlap.
...
3. **Promote**: When a lesson proves universally valuable, it **SHOULD** be proposed for inclusion in the relevant hand-authored instruction file (requires human approval).
4. **Retire**: Lessons that are no longer relevant (fixed by tooling, superseded by a promoted rule) **SHOULD** be removed during periodic review.
