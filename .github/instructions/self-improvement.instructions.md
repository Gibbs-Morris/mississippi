...
- When resolving a problem reveals a failure mode, workaround, or process gap that could recur across multiple tasks—not merely the same task's exact code or data—the agent **MUST** record the validated lesson in the appropriate `self-taught-<domain>.instructions.md` file after resolution. Why: Generalizable learning belongs in the shared instruction context, not only in a task-local record.
