...
- Namespaces **MUST** be feature-oriented (no `Services/Models` silos), max five PascalCase segments, no underscores; abbreviations **MUST** be industry-standard only. Why: Improves discoverability.
...
- Public (and internal when exposed) symbols **MUST** have XML docs with `<summary>` (imperative voice), `<param>`/`<typeparam>`/`<returns>` as applicable; text **MUST** be factual (no TODOs) and `<example>` snippets **MUST** compile. Why: Ensures IntelliSense and docs quality.
...
- Derive namespaces from company/product/feature; keep <=5 segments.
