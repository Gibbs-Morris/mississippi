...
- `SnapshotEnvelope` preserves its Orleans alias and serialized member IDs `[Id(0..3)]`. Cosmos mapping stores its
  payload, content type, and payload-size fields. On read, `SnapshotEnvelope.ReducerHash` is derived from the stored
  `SnapshotStreamKey` reducer hash; the document's `reducersHash` is taken from `SnapshotStreamKey`.
