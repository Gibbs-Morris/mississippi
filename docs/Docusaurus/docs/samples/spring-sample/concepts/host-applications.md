...
The client-side generators now target `IReservoirBuilder`, so generated client features compose on the same Reservoir builder as hand-written client features. Domain-level client composition methods may also be generated by the domain client generator, but the Spring sample on this branch shows the per-feature builder composition explicitly.
