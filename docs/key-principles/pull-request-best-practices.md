...
- Add one complete behavior within a feature
- Fix a specific bug
...
the behavior in that PR; a future layer is not a substitute for a complete change.
...
prioritizes a self-contained change and reviewer judgment. File spread and
...
| Step | Purpose |
|---|---|
| **Read the logical diff; summarize autogenerated lockfiles** | Catch unintended changes, debug statements, and commented-out code; validate lockfile regeneration and dependency intent |
...
behavior against repository evidence; remove speculative generalization, repeated
...
```text
main ← PR 1 (tested prerequisite refactor) ← PR 2 (behavior + tests/docs) ← PR 3 (next behavior + tests/docs)
```
...
When merge is authorized, `gh stack merge <target> --yes` can land a ready prefix
...
explains native protections and merge behavior; the [canonical repository workflow](../../.github/instructions/pr-size-and-stacking.instructions.md#github-and-gh-stack-workflow)
...
| **Google Engineering Practices** | [Small CLs](https://google.github.io/eng-practices/review/developer/small-cls.html): self-contained changes, tests, and size judgment. |
