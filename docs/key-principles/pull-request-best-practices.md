...
For this repository, follow the canonical [PR size and stacked delivery
policy](../../.github/instructions/pr-size-and-stacking.instructions.md): one
logical change per PR, targeting 600 changed lines or fewer, with justified
exceptions and a CI/review gate before starting each dependent layer.
...
- Add one complete behaviour within a feature
- Fix a specific bug
...
- A database migration AND unrelated business logic changes

Keep tightly coupled changes together when separating them would hide correctness
or leave broken consumers. Include the tests and documentation needed to validate
the behaviour in that PR; a future layer is not a substitute for a complete change.

### Size Matters

Aim for **600 changed lines or fewer**, including tests and documentation, measured
as additions plus deletions against the PR's immediate base, excluding autogenerated
lockfile changes. Smaller is welcome.
600 is a target, not a minimum, hard limit, or scientifically established optimum.

[Google's Small CLs guidance](https://google.github.io/eng-practices/review/developer/small-cls.html)
prioritises a self-contained change and reviewer judgement. File spread and
complexity also matter: a short diff touching many concepts may be harder to
review than a larger mechanical change. Avoid treating line-count bands as
predictions of review quality.

Above 600 lines, explain why the change belongs together, why splitting would
make review or correctness worse, and which files to review in what order.
Report autogenerated lockfile churn separately from logical review size and the
raw total; it does not require a size exception or line-by-line code review.
Review the intended dependency changes and verify regeneration and applicable
restore/build checks instead. Identify other mechanical/generated changes separately.
The reviewer decides whether the explanation makes the change manageable.
Do not remove tests, compress formatting, or add temporary scaffolding to hit
a number. Split unrelated concerns even when their combined diff is small.
...
| Step | Purpose |
|---|---|
| **Read the logical diff; summarise autogenerated lockfiles** | Catch unintended changes, debug statements, and commented-out code; validate lockfile regeneration and dependency intent |
...
For AI-assisted work, the author owns every line and every claim. Verify APIs and
behaviour against repository evidence; remove speculative generalisation, repeated
explanations, unrelated fixes, and tests that assert nothing useful. PR descriptions
report actual validation and remaining uncertainty. Generated text does not earn
an exemption from review. [Google's review checklist](https://google.github.io/eng-practices/review/reviewer/looking-for.html)
specifically calls for checking complexity, tests, documentation, and unnecessary
...
Plan large features as **native GitHub stacked PRs**, using `gh stack` and the
[official gh-stack skill](https://github.com/github/gh-stack/blob/main/skills/gh-stack/SKILL.md).
Read its [stack-design reference](https://github.com/github/gh-stack/blob/main/skills/gh-stack/references/stack-design.md)
before choosing layers. Put one complete outcome in each layer:

```text
main ← PR 1 (tested prerequisite refactor) ← PR 2 (behaviour + tests/docs) ← PR 3 (next behaviour + tests/docs)
```

Each PR targets its predecessor so reviewers see that layer's diff. Native stacks
are linear and stay within one repository; cross-fork stacks are unsupported.
Independent work belongs in separate PRs or stacks.

Implement one layer, submit it, pass all applicable CI/CD, obtain required
approvals, and resolve all feedback before creating or implementing its dependent
successor. Planning later layers can continue while review is pending. An approved
parent can remain unmerged so the stack can land together. This sequential gate
is Mississippi policy; GitHub and Google also describe working concurrently on
dependent layers.

If a lower layer changes, fix its owning branch, propagate the change with the
skill's rebase/sync workflow, and revalidate affected layers. Silence, old passing
checks, outdated threads, or a local completion marker do not establish readiness.
See the [advancement gate](../../.github/instructions/pr-size-and-stacking.instructions.md#advancement-gate)
for the exact evidence required.

When merge is authorised, `gh stack merge <target> --yes` can land a ready prefix
or the whole stack while retaining individual review units. A PR target includes
all unmerged ancestors; a stack target includes every layer. Recheck all included
PRs first. Merge queues may land separate groups; grouped merge does not guarantee
atomic deployment. The [GitHub documentation](https://docs.github.com/en/pull-requests/get-started/about-stacked-prs)
explains native protections and merge behaviour; the [canonical repository workflow](../../.github/instructions/pr-size-and-stacking.instructions.md#github-and-gh-stack-workflow)
contains setup, command examples, and preview limitations.
...
It is not Mississippi's delivery workflow: complete the applicable CI and review
gates before merge, including when landing a stack together.
...
|---|---|---|
| **Large PRs** | Review scope becomes difficult to reason about | Target 600 changed lines or fewer; justify larger coherent changes and stack dependent outcomes |
...
| **Rubber-stamp reviews** | Approving without reading | Verify correctness and evidence; do not manufacture comments to satisfy a quota |
...
| **Google Engineering Practices** | [Small CLs](https://google.github.io/eng-practices/review/developer/small-cls.html): self-contained changes, tests, and size judgement. |
| **GitLab Handbook** | [Iteration](https://handbook.gitlab.com/handbook/engineering/workflow/iteration/): useful increments and trade-offs between vertical and horizontal slicing. |
| **GitHub** | [Native stacks announcement, July 30, 2026](https://github.blog/changelog/2026-07-30-stacked-pull-requests-are-now-in-public-preview/) and [gh-stack skill](https://github.com/github/gh-stack/blob/main/skills/gh-stack/SKILL.md). Preview guidance checked September 8, 2026. |
