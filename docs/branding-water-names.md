...
| Ripples | `Mississippi.Ripples` packages and `docs/architecture/*ripples*` | Blazor state management for UX projections: subscription lifecycle + automatic real-time updates (Server and WASM). |
| Ripple | `IRipple<T>` | A live subscription to one projection instance (entity) that exposes `Current`, connection/load state, and change notifications. |
...
- Projection changes ripple outward to clients via HTTP + SignalR, with Ripples providing a consistent Blazor developer experience.
...
- Real-time UX delivery: projection APIs + SignalR notifications, plus the Ripples library for reactive Blazor components (Server + WASM).
...
- `docs/architecture/c1-ripples-context.md`
...
- `src/Ripples.Abstractions/IRipple.cs`
