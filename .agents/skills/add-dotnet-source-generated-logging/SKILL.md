...
- Keep the generator's enabled check by default. It cannot prevent argument expressions being evaluated at the call site: guard expensive **logging-only** preparation with a matching check on the same logger. For a fixed `[LoggerMessage(Level = LogLevel.Information)]`, use `logger.IsEnabled(LogLevel.Information)`; for a dynamic level, use `logger.IsEnabled(level)` with the same value passed to the generated method. Do not move required business work inside that guard. Use `SkipEnabledCheck` only when every caller demonstrably supplies the matching check.
