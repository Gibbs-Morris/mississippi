...
function Get-AgentDoctorReport {
    ...
    if ($profiles -contains 'Core' -or $profiles -contains 'Spring') {
        $dotnet = Invoke-DoctorProbe -Name 'dotnet-version' -FilePath 'dotnet' -Arguments @('--version') -ProbeOverrides $ProbeOverrides
        ...
        $git = Invoke-DoctorProbe -Name 'git-root' -FilePath 'git' -Arguments @('-C', $root, 'rev-parse', '--show-toplevel') -ProbeOverrides $ProbeOverrides
    ...
    if ($profiles -contains 'Docs') {
        ...
        $node = Invoke-DoctorProbe -Name 'node-version' -FilePath 'node' -Arguments @('--version') -ProbeOverrides $ProbeOverrides
        $npm = Invoke-DoctorProbe -Name 'npm-version' -FilePath 'npm' -Arguments @('--version') -ProbeOverrides $ProbeOverrides
    ...
    if ($profiles -contains 'Spring') {
        $docker = Invoke-DoctorProbe -Name 'docker-ostype' -FilePath 'docker' -Arguments @('info', '--format', '{{.OSType}}') -ProbeOverrides $ProbeOverrides
    ...
    if ($profiles -contains 'GitHub') {
        $remote = Invoke-DoctorProbe -Name 'git-remote' -FilePath 'git' -Arguments @('-C', $root, 'config', '--get', 'remote.origin.url') -ProbeOverrides $ProbeOverrides
        $gh = Invoke-DoctorProbe -Name 'github-repository' -FilePath 'gh' -Arguments @('repo', 'view', '--json', 'nameWithOwner') -ProbeOverrides $ProbeOverrides
