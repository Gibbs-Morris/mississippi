...
> Tip: when experimenting interactively you can `Import-Module ./eng/src/agent-scripts/RepositoryAutomation.psm1` and call the functions directly (for example `Invoke-MississippiSolutionBuild -Configuration Debug`). Keep the module loaded while an execution lease is active.
