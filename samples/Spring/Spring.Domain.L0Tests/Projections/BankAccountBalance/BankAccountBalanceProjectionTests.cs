...
public sealed class BankAccountBalanceProjectionTests
{
    ...
    public void ScenarioOpenAccountAndDepositBalanceReflectsTotal()
    {
        ...
        CreateHarness()
            .CreateScenario()
            .When(new AccountOpened { HolderName = "John Doe", InitialDeposit = 100.00m })
            .When(new FundsDeposited { Amount = 50.00m })
            .When(new FundsDeposited { Amount = 25.00m })
    ...
    public void ScenarioOpenAccountAndWithdrawBalanceReflectsTotal()
    {
        CreateHarness()
            .CreateScenario()
            .When(new AccountOpened { HolderName = "Jane Doe", InitialDeposit = 500.00m })
            .When(new FundsWithdrawn { Amount = 100.00m })
            .When(new FundsWithdrawn { Amount = 50.00m })
            .ThenAssert(projection =>
            {
                projection.Balance.Should().Be(350.00m);
            });
    }
    ...
    public void ScenarioFullLifecycleBalanceIsCorrect()
    {
        CreateHarness()
            .CreateScenario()
            .When(new AccountOpened { HolderName = "Full Lifecycle", InitialDeposit = 1000.00m })
            .When(new FundsDeposited { Amount = 500.00m })
            .When(new FundsWithdrawn { Amount = 200.00m })
            .When(new FundsDeposited { Amount = 100.00m })
            .When(new FundsWithdrawn { Amount = 50.00m })
            .ThenShouldBe(new BankAccountBalanceProjection
            {
    ...
    public void ScenarioGivenPriorEventsWhenNewEventThenCorrectState()
    {
        ...
        CreateHarness()
            .CreateScenario()
            .Given(
                new AccountOpened { HolderName = "Prior State", InitialDeposit = 100.00m },
                new FundsDeposited { Amount = 50.00m })
            .When(new FundsDeposited { Amount = 25.00m })
            .ThenAssert(projection =>
            {
                projection.Balance.Should().Be(175.00m);
    ...
    public void ApplyEventsWithMultipleEventsReturnsCorrectProjection()
    {
        ...
        BankAccountBalanceProjection result = harness.ApplyEvents(
            new AccountOpened { HolderName = "Quick Test", InitialDeposit = 200.00m },
            new FundsDeposited { Amount = 100.00m },
            new FundsWithdrawn { Amount = 50.00m });
    ...
    public void ScenarioThenShouldSatisfyValidatesPredicate()
    {
        CreateHarness()
            .CreateScenario()
            .When(new AccountOpened { HolderName = "Predicate Test", InitialDeposit = 100.00m })
            .ThenShouldSatisfy(
    ...
    public void ScenarioAppliedEventsTracksEventHistory()
    {
        ...
        ProjectionScenario<BankAccountBalanceProjection> scenario = CreateHarness()
            .CreateScenario()
            .When(new AccountOpened { HolderName = "History", InitialDeposit = 100.00m })
            .When(new FundsDeposited { Amount = 50.00m });
