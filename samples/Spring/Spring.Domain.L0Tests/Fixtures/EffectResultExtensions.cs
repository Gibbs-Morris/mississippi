...
public static class EffectResultExtensions
{
    ...
    )
    {
        ...
        result.WasFlagged.Should().BeFalse(
            because.Length > 0
                ? because
                : $"deposit of {result.DepositAmount:C} should not exceed AML threshold");
    ...
    )
    {
        ...
        result.WasFlagged.Should().BeTrue(
            because.Length > 0
                ? because
                : $"deposit of {result.DepositAmount:C} should exceed AML threshold");
    ...
    /// <returns>The result for chaining.</returns>
    public static HighValueEffectResult WithAccountId(this HighValueEffectResult result, string expectedAccountId)
    {
        ArgumentNullException.ThrowIfNull(result);

        result.DispatchedCommands.Should().ContainSingle();
        FlagTransaction command = result.DispatchedCommands[0].Command.Should()
            .BeOfType<FlagTransaction>()
            .Subject;
    ...
    /// <returns>The result for chaining.</returns>
    public static HighValueEffectResult WithAmount(this HighValueEffectResult result, decimal expectedAmount)
    {
        ArgumentNullException.ThrowIfNull(result);
