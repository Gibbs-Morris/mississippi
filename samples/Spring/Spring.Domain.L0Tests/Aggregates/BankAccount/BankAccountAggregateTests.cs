...
public sealed class BankAccountAggregateTests
{
    ...
    public void DepositToExistingAccountUpdatesBalance()
    {
        ...
        CreateHarness()
            .CreateScenario()
            .Given(new AccountOpened { HolderName = "Bob", InitialDeposit = 100m })
    ...
    public void WithdrawFromExistingAccountUpdatesBalance()
    {
        ...
        CreateHarness()
            .CreateScenario()
            .Given(new AccountOpened { HolderName = "Carol", InitialDeposit = 200m })
            .When(new WithdrawFunds { Amount = 75m })
    ...
    public void MultipleTransactionsMaintainCorrectBalance()
    {
        ...
        AggregateScenario<BankAccountAggregate> scenario = CreateHarness()
            .CreateScenario()
            .Given(
                new AccountOpened { HolderName = "Dave", InitialDeposit = 100m },
                new FundsDeposited { Amount = 50m },
                new FundsDeposited { Amount = 25m },
                new FundsWithdrawn { Amount = 30m });

        // Act & Assert
        scenario
            .When(new DepositFunds { Amount = 100m })
            .ThenState(s =>
            {
    ...
    public void WithdrawMoreThanBalanceFails()
    {
        ...
        AggregateScenario<BankAccountAggregate> scenario = CreateHarness()
            .CreateScenario()
            .Given(new AccountOpened { HolderName = "Eve", InitialDeposit = 50m })
            .When(new WithdrawFunds { Amount = 100m })
    ...
    public void OpenAlreadyOpenAccountFails()
    {
        ...
        AggregateScenario<BankAccountAggregate> scenario = CreateHarness()
            .CreateScenario()
            .Given(new AccountOpened { HolderName = "Frank", InitialDeposit = 100m })
    ...
    public void StateAfterFailedCommandRemainsUnchanged()
    {
        ...
        AggregateScenario<BankAccountAggregate> scenario = CreateHarness()
            .CreateScenario()
            .Given(new AccountOpened { HolderName = "Hank", InitialDeposit = 100m });

        BankAccountAggregate stateBefore = scenario.State;

        // Act - attempt invalid withdrawal
        scenario.When(new WithdrawFunds { Amount = 200m });
    ...
    public void CompleteAccountLifecycleScenario()
    {
        ...
        scenario = harness.CreateScenario()
            ...
            .When(new DepositFunds { Amount = 500m })
            .ThenSucceeds();

        scenario = harness.CreateScenario()
            .Given(scenario.AllAppliedEvents.ToArray())
            .When(new DepositFunds { Amount = 250m })
            .ThenSucceeds();

        // Make withdrawal
        harness.CreateScenario()
            .Given(scenario.AllAppliedEvents.ToArray())
            .When(new WithdrawFunds { Amount = 200m })
            .ThenState(s =>
            {
