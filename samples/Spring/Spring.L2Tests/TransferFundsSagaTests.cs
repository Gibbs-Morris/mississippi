...
public sealed class TransferFundsSagaTests
{
    ...
    public async Task TransferFundsCompensatesWhenDestinationClosed()
    {
        ...
        await OpenAccountAsync(client, destId, "ClosedReceiver", 100m);

        // Close the destination account
        await CloseAccountAsync(client, destId);
        ...
        SagaStatusResponse? status = await WaitForSagaFailureAsync(
            client,
            sagaId,
            timeout: TimeSpan.FromSeconds(60)); // Extra time for compensation
        ...
        BankAccountBalanceResponse? sourceBalance = await GetBalanceAsync(client, sourceId);
        sourceBalance.Should().NotBeNull();
        sourceBalance!.Balance.Should().Be(
            sourceInitial,
            "source balance should be restored after compensation");
    }
    ...
    {
        using HttpResponseMessage response = await client.PostAsJsonAsync(
            new Uri($"api/aggregates/bank-account/{accountId}/close", UriKind.Relative),
            new { });
        response.StatusCode.Should().Be(HttpStatusCode.OK, $"closing account {accountId} should succeed");
    }
