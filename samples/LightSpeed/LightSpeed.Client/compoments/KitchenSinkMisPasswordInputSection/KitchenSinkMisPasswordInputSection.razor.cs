...
public sealed partial class KitchenSinkMisPasswordInputSection : StoreComponent
{
    ...
    private IReadOnlyList<string> PasswordEvents =>
        Select<MisPasswordInputKitchenSinkState, IReadOnlyList<string>>(
            MisPasswordInputKitchenSinkSelectors.GetEventLog);

    private MisPasswordInputViewModel PasswordModel =>
        Select<MisPasswordInputKitchenSinkState, MisPasswordInputViewModel>(
            MisPasswordInputKitchenSinkSelectors.GetViewModel);
    ...
    private static string FormatAction(
        ...
        action switch
        {
            MisPasswordInputInputAction input => $"intent={input.IntentId}, value.len={input.Value?.Length ?? 0}",
            ...
            MisPasswordInputFocusedAction focused => $"intent={focused.IntentId}",
            MisPasswordInputBlurredAction blurred => $"intent={blurred.IntentId}",
    ...
    )
    {
        if (action is MisSelectChangedAction changedAction &&
            ...
            Enum.TryParse(changedAction.Value, out MisPasswordInputState state))
        {
