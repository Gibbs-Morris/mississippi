...
public sealed partial class KitchenSinkMisRadioGroupSection : StoreComponent
{
    ...
    private static IReadOnlyList<MisSelectOptionViewModel> RadioGroupStateOptions { get; } = Enum
        .GetValues<MisRadioGroupState>()
    ...
    private static string FormatAction(
        ...
        action switch
        {
            MisRadioGroupInputAction input => $"intent={input.IntentId}, value={input.Value}",
            MisRadioGroupChangedAction changed => $"intent={changed.IntentId}, value={changed.Value}",
    ...
    )
    {
        ...
        switch (changedAction.IntentId)
        {
            case "prop-value":
                Dispatch(new SetMisRadioGroupValueAction(changedAction.Value));
                break;
            case "prop-state" when Enum.TryParse(changedAction.Value, true, out MisRadioGroupState state):
    ...
    )
    {
        ...
        if (action is MisTextInputInputAction inputAction)
        {
            switch (inputAction.IntentId)
            {
                case "prop-intentid":
                    Dispatch(
                        new SetMisRadioGroupIntentIdAction(
                            !string.IsNullOrWhiteSpace(inputAction.Value)
                                ? inputAction.Value
                                : "kitchen-sink.mis-radio-group"));
                    break;
                case "prop-arialabel":
                    Dispatch(
                        new SetMisRadioGroupAriaLabelAction(
                            string.IsNullOrWhiteSpace(inputAction.Value) ? null : inputAction.Value));
                    break;
                case "prop-title":
                    Dispatch(
                        new SetMisRadioGroupTitleAction(
                            string.IsNullOrWhiteSpace(inputAction.Value) ? null : inputAction.Value));
                    break;
                case "prop-cssclass":
                    Dispatch(
                        new SetMisRadioGroupCssClassAction(
                            string.IsNullOrWhiteSpace(inputAction.Value) ? null : inputAction.Value));
                    break;
            }
        }
