...
/// <summary>
///     Azurite-backed trust slice proving canonical Blob snapshot registration, gzip framing, and Blob-backed restart hydration.
/// </summary>
public sealed class BlobSnapshotTrustSliceTests
{
    ...
    public async Task LargeSnapshotShouldSurviveRestartWithGzipAndNonDefaultSerializer()
    {
        ...
        inspection.PayloadPrefix.Should().Equal((byte)0x1F, (byte)0x8B);

        string blobBackedMarker = $"blob-backed-{Guid.NewGuid():N}";
        string blobBackedPayload = CreateLargePayload(payload.Length, salt: 17);
        blobBackedPayload.Should().NotBe(payload, "the restart assertion must distinguish Blob reload from event replay");
    ...
    ) => compressionAlgorithm switch
    {
        ...
        _ => throw new InvalidOperationException($"Unsupported compression algorithm '{compressionAlgorithm}'."),
    };
    ...
    )
    {
        ...
        PersistedSnapshotBlobHeader header = JsonSerializer.Deserialize<PersistedSnapshotBlobHeader>(
                                            ...
                                            ?? throw new InvalidOperationException("The snapshot blob header could not be parsed.");
    ...
    private sealed record BlobFrameInspection(
        PersistedSnapshotBlobHeader Header,
        IReadOnlyList<byte> PayloadPrefix);
