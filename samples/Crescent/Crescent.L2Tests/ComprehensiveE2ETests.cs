...
#pragma warning restore CA1515
{
    ...
    public async Task LargeOperationSequenceMaintainsCorrectState()
    {
        ...
        output.WriteLine($"[Test] Large sequence completed: Count={projection.CurrentCount}, Ops={projection.TotalOperations}");
    ...
    public async Task ResetAndRecoveryProjectsCorrectly()
    {
        ...
        // Act - Reset to 1000
        OperationResult resetResult = await counter.ExecuteAsync(new ResetCounter { NewValue = 1000 });
        ...
        output.WriteLine($"[Test] Reset and recovery: Count={projection.CurrentCount}, Ops={projection.TotalOperations}");
    ...
    public async Task RapidSequentialUpdatesMaintainCorrectOrder()
    {
        ...
        for (int i = 1; i <= 5; i++)
        {
            OperationResult result = await counter.ExecuteAsync(new IncrementCounter { Amount = i });
            result.Success.Should().BeTrue($"Increment({i}) should succeed");
        }
        ...
        for (int i = 1; i <= 2; i++)
        {
            await counter.ExecuteAsync(new DecrementCounter { Amount = i });
        }
    ...
    public async Task ProjectionAfterDeactivationReturnsSameState()
    {
        ...
        output.WriteLine($"[Test] After: Count={afterDeactivation.CurrentCount}, Ops={afterDeactivation.TotalOperations}");
