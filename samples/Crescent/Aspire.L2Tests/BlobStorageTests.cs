...
/// <summary>
///     Integration tests for Azure Blob Storage operations using the Aspire Azurite emulator.
/// </summary>
...
#pragma warning restore CA1515
{
    ...
    public BlobStorageTests(AspireFixture fixture)
    {
    ...
    public async Task WriteBlobShouldSucceed()
    {
        ...
        response.Value.Should().NotBeNull(
            because: "the upload should return content info");
#pragma warning disable IDISP004 // Don't ignore created IDisposable - GetRawResponse returns wrapper that doesn't need disposal
        response.GetRawResponse().Status.Should().Be(
            201,
            because: "HTTP 201 Created indicates successful upload");
#pragma warning restore IDISP004

        // Verify the blob exists
        bool exists = await blobClient.ExistsAsync();
        exists.Should().BeTrue(
            because: "the blob should exist after upload");
    }
    ...
    public async Task ReadBlobShouldReturnWrittenContent()
    {
        ...
        // Act
        Azure.Response<BlobDownloadResult> downloadResponse = await blobClient.DownloadContentAsync();
        string actualContent = downloadResponse.Value.Content.ToString();

        // Assert
        actualContent.Should().Be(
            expectedContent,
            because: "the downloaded content should match what was uploaded");
    }
    ...
    public async Task ReadNonExistentBlobShouldThrowNotFoundException()
    {
        ...
        // Assert
        await act.Should()
            .ThrowAsync<RequestFailedException>(because: "the blob does not exist")
    ...
    public async Task ListBlobsShouldReturnUploadedBlobs()
    {
        ...
        // Assert
        listedBlobNames.Should().HaveCount(
            3,
            because: "we uploaded 3 blobs with the matching prefix");
        listedBlobNames.Should().BeEquivalentTo(
            uploadedBlobNames,
            because: "the listed blobs should match the uploaded blobs");
    ...
    public async Task DeleteBlobShouldRemoveBlob()
    {
        ...
        existsBefore.Should().BeTrue(because: "the blob should exist before deletion");
        ...
        existsAfter.Should().BeFalse(because: "the blob should not exist after deletion");
    }
...
#pragma warning restore CA5394
        ...
        // Assert
        actualContent.Should().BeEquivalentTo(
            expectedContent,
            because: "binary content should be preserved exactly");
    }
}
