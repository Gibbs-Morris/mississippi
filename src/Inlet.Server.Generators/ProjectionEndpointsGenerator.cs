...
public sealed class ProjectionEndpointsGenerator : IIncrementalGenerator
{
    ...
    /// <summary>
    ///     Extracts authorization configuration from the GenerateProjectionEndpoints attribute.
    /// </summary>
    private static AuthorizationInfo ExtractProjectionAuthorization(
        AttributeData generateAttr
    )
    {
        bool allowAnonymous = TypeAnalyzer.GetBooleanProperty(generateAttr, "AllowAnonymous", false);
        bool requiresAuth = TypeAnalyzer.GetBooleanProperty(generateAttr, "RequiresAuthentication", false);
        string? authorizeRoles = TypeAnalyzer.GetStringProperty(generateAttr, "AuthorizeRoles");
        string? authorizePolicy = TypeAnalyzer.GetStringProperty(generateAttr, "AuthorizePolicy");

        return new AuthorizationInfo(allowAnonymous, requiresAuth, authorizeRoles, authorizePolicy);
    }
    ...
    )
    {
        ...
        if (projection.Authorization.HasAuthorization)
        {
        ...
        EmitAuthorizationAttribute(sb, projection.Authorization);
    ...
    )
    {
        ...
        for (int j = 0; j < mappedProps.Length; j++)
        {
            ...
            string paramName = ToCamelCase(prop.Name) + MapperSuffix;
            sb.AppendLine($"{prop.Name}{MapperSuffix} = {paramName};");
        }
    ...
    )
    {
        ...
        if (!TypeAnalyzer.GetBooleanProperty(generateAttr, "GenerateController"))
        {
            return null;
        }
    ...
    {
        // Use the compilation provider to scan both current and referenced types and extract security settings
        IncrementalValueProvider<(List<ProjectionInfo> Projections, SecurityEnforcementSettings? SecuritySettings)>
            projectionsWithSecurityProvider = context.CompilationProvider.Select((
                compilation,
                _
            ) =>
            {
                ...
                SecurityEnforcementSettings? securitySettings =
                    AuthorizationValidator.GetSecurityEnforcementSettings(compilation);
                return (projections, securitySettings);
            });

        // Register source output
        context.RegisterSourceOutput(
            projectionsWithSecurityProvider,
            static (
                spc,
                data
            ) =>
            {
                ...
                foreach (ProjectionInfo projection in data.Projections)
                {
                    ...
                    ValidateAndReportDiagnostics(spc, projection, data.SecuritySettings);
    ...
    /// <summary>
    ///     Validates projection authorization and reports diagnostics.
    /// </summary>
    ...
    )
    {
        ...
        if (securitySettings.IsTypeExempt(projection.Model.FullTypeName))
        {
            return;
        }

        AuthorizationInfo auth = projection.Authorization;
            ...
            if (securitySettings.TreatAnonymousAsError)
            {
                context.ReportDiagnostic(
                    Diagnostic.Create(
                        ...
                        projection.Model.Symbol.Locations.FirstOrDefault(),
                        projection.Model.TypeName));
            }
            else
            {
                context.ReportDiagnostic(
