...
public sealed class ProjectionClientRegistrationGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        sb.AppendLine();
        sb.AppendLine("using Mississippi.Inlet.Client;");
        sb.AppendLine("using Mississippi.Inlet.Client.Abstractions;");
        ...
        sb.AppendLine("using Mississippi.Reservoir.Core;");
        sb.AppendLine();
        ...
        sb.AppendLine("        reservoir.AddFeature<ProjectionsFeatureState>(feature =>");
        sb.AppendLine("        {");
        ...
        projectionDtos.Select(dto => dto.TypeName)
            ...
            .ForEach(typeName =>
            {
                ...
                sb.AppendLine(
                    $"        feature.AddReducer<ProjectionsFeatureState, ProjectionLoadingAction<{typeName}>>(");
                sb.AppendLine($"            ProjectionsReducer.ReduceLoading<{typeName}>);");
                sb.AppendLine(
                    $"        feature.AddReducer<ProjectionsFeatureState, ProjectionLoadedAction<{typeName}>>(");
                sb.AppendLine($"            ProjectionsReducer.ReduceLoaded<{typeName}>);");
                sb.AppendLine(
                    $"        feature.AddReducer<ProjectionsFeatureState, ProjectionUpdatedAction<{typeName}>>(");
                sb.AppendLine($"            ProjectionsReducer.ReduceUpdated<{typeName}>);");
                sb.AppendLine(
                    $"        feature.AddReducer<ProjectionsFeatureState, ProjectionErrorAction<{typeName}>>(");
                sb.AppendLine($"            ProjectionsReducer.ReduceError<{typeName}>);");
                sb.AppendLine(
                    $"        feature.AddReducer<ProjectionsFeatureState, ProjectionConnectionChangedAction<{typeName}>>(");
        ...
        // Register projection paths explicitly (replaces ScanProjectionDtos)
        sb.AppendLine("        // Explicit projection path registrations (generated from [ProjectionPath] attributes)");
        ...
        sb.AppendLine("                nameof(reservoir));");
        projectionDtos.OrderBy(dto => dto.TypeName, StringComparer.Ordinal)
            .ToList()
            .ForEach(dto =>
            {
                sb.AppendLine(
                    $"        builder.Services.AddSingleton<IConfigureProjectionRegistry>(new ProjectionPathRegistration<{dto.TypeName}>(\"{EscapeString(dto.Path)}\"));");
            });
        sb.AppendLine();
