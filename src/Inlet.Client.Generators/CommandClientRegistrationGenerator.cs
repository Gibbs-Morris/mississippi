...
public sealed class CommandClientRegistrationGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        sb.AppendLine($"///     Adds the {aggregate.AggregateName} aggregate feature to the Reservoir builder.");
        sb.AppendLine("/// </summary>");
        ...
        {
            sb.AppendLine(
                $"builder.Services.AddTransient<IMapper<{commandName}Action, {commandName}RequestDto>, {commandName}ActionMapper>();");
        }
        ...
        foreach (string commandName in aggregate.CommandNames.OrderBy(n => n))
        {
            ...
            sb.AppendLine($"feature.AddReducer<{stateTypeName}, {commandName}ExecutingAction>(");
            sb.IncreaseIndent();
            sb.AppendLine($"{reducersTypeName}.{commandName}Executing);");
            sb.DecreaseIndent();
            sb.AppendLine($"feature.AddReducer<{stateTypeName}, {commandName}SucceededAction>(");
            sb.IncreaseIndent();
            sb.AppendLine($"{reducersTypeName}.{commandName}Succeeded);");
            sb.DecreaseIndent();
            sb.AppendLine($"feature.AddReducer<{stateTypeName}, {commandName}FailedAction>(");
            sb.IncreaseIndent();
        ...
        foreach (string commandName in aggregate.CommandNames.OrderBy(n => n))
        {
            sb.AppendLine($"feature.AddActionEffect<{stateTypeName}, {commandName}ActionEffect>();");
        }
