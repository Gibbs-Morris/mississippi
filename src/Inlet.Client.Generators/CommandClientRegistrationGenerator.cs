...
public sealed class CommandClientRegistrationGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        foreach (string commandName in aggregate.CommandNames.OrderBy(n => n))
        {
            ...
            sb.AppendLine(
                $"feature.AddReducer<{commandName}ExecutingAction>({reducersTypeName}.{commandName}Executing);");
            sb.AppendLine(
                $"feature.AddReducer<{commandName}SucceededAction>({reducersTypeName}.{commandName}Succeeded);");
            sb.AppendLine($"feature.AddReducer<{commandName}FailedAction>({reducersTypeName}.{commandName}Failed);");
            sb.AppendLine();
        }
        ...
        foreach (string commandName in aggregate.CommandNames.OrderBy(n => n))
        {
            sb.AppendLine($"feature.AddActionEffect<{commandName}ActionEffect>();");
        }
