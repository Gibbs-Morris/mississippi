...
public sealed class InletClientCompositeGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        sb.AppendUsing("Mississippi.Sdk.Client");
        sb.AppendLine();
        ...
        sb.AppendLine("/// </summary>");
        sb.AppendLine("/// <param name=\"builder\">The Mississippi client builder.</param>");
        sb.AppendLine("/// <returns>The Mississippi client builder for chaining.</returns>");
        sb.AppendLine($"public static MississippiClientBuilder {methodName}(");
        sb.IncreaseIndent();
        sb.AppendLine("this MississippiClientBuilder builder");
        sb.DecreaseIndent();
        sb.AppendLine(")");
        sb.OpenBrace();
        sb.AppendLine($"return {methodName}(builder, null);");
        sb.CloseBrace();
        sb.AppendLine();
        sb.AppendLine("/// <summary>");
        sb.AppendLine(
        ...
        sb.IncreaseIndent();
        sb.AppendLine("this MississippiClientBuilder builder,");
        sb.AppendLine("Action<InletBlazorSignalRBuilder>? configureSignalR");
        sb.DecreaseIndent();
        sb.AppendLine(")");
        sb.OpenBrace();
        ...
        if (info.AggregateNames.Count > 0)
        {
            ...
            {
                sb.AppendLine($"builder.Services.Add{aggregateName}AggregateFeature();");
            }
        ...
        sb.AppendLine("builder.Services.AddReservoirBlazorBuiltIns();");
        sb.AppendLine();

        // Inlet client + SignalR
        sb.AppendLine("// Inlet client with SignalR for real-time projection updates");
        sb.AppendLine("builder.Services.AddInletClient();");

        // Emit explicit projection DTO registrations instead of runtime scanning
        if (info.ProjectionDtos.Count > 0)
        {
            sb.AppendLine("builder.Services.AddInletBlazorSignalR(signalR =>");
            sb.AppendLine("{");
            sb.IncreaseIndent();
            sb.AppendLine($"signalR.WithHubPath(\"{info.HubPath}\");");
            sb.AppendLine("signalR.RegisterProjectionDtos(registry =>");
            sb.AppendLine("{");
            sb.IncreaseIndent();
            ...
            sb.DecreaseIndent();
            sb.AppendLine("});");
            sb.AppendLine("configureSignalR?.Invoke(signalR);");
            sb.DecreaseIndent();
            sb.AppendLine("});");
        }
        else
