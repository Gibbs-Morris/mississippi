    ...
    where TProjection : class
{
    ...
    public async Task SubscribeAsync(string entityId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            ClientRippleLoggerExtensions.SubscribedToProjection(
                Logger,
                projectionTypeName,
                entityId);
        }
    ...
    private async Task UnsubscribeInternalAsync(CancellationToken cancellationToken)
    {
        if (EntityId == null)
        {
            return;
        ...
        await SignalRConnection.UnsubscribeAsync(
            typeof(TProjection).Name,
            EntityId,
            cancellationToken).ConfigureAwait(false);

        ClientRippleLoggerExtensions.UnsubscribedFromProjection(
            Logger,
            typeof(TProjection).Name,
            EntityId);
    ...
    private async Task FetchProjectionAsync(string entityId, CancellationToken cancellationToken)
    {
        ...
        {
            request.Headers.IfNoneMatch.Add(new System.Net.Http.Headers.EntityTagHeaderValue(currentETag));
        }
        ...
        if (response.Headers.TryGetValues("X-Projection-Version", out System.Collections.Generic.IEnumerable<string>? versions))
        {
    ...
    private void OnVersionUpdated(long newVersion)
    {
        if (newVersion > (Version ?? -1) && EntityId != null)
        {
            ClientRippleLoggerExtensions.ReceivedVersionUpdate(
                Logger,
                typeof(TProjection).Name,
                EntityId,
                newVersion);
    ...
    {
        RaiseChanged();
        ErrorOccurred?.Invoke(this, new RippleErrorEventArgs(ex));
    }
}
