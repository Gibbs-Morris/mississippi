...
/// </summary>
internal sealed class CacheEntryGrain
    : IGrainBase,
      ICacheEntryGrain
{
    /// <summary>
    ...
    public Task<CacheEntryData?> GetAsync()
    {
        ...
        {
            Logger.CacheEntryExpired(this.GetPrimaryKeyString(), "absolute");
            return Task.FromResult<CacheEntryData?>(null);
        }
        ...
        if (data.SlidingExpiration.HasValue)
        {
            ...
            if (expiresAt <= now)
            {
                Logger.CacheEntryExpired(this.GetPrimaryKeyString(), "sliding");
                return Task.FromResult<CacheEntryData?>(null);
            }
        }

        Logger.CacheEntryRetrieved(this.GetPrimaryKeyString(), data.Value.Length);
    ...
    public async Task RefreshAsync()
    {
        ...
        if (data.SlidingExpiration.HasValue)
        {
            State.State.Data = data with
            {
                LastAccessTime = TimeProvider.GetUtcNow(),
            };
            await State.WriteStateAsync();
            Logger.CacheEntryRefreshed(this.GetPrimaryKeyString());
        }
    }
    ...
    public async Task RemoveAsync()
    {
        ...
        Logger.CacheEntryRemoved(this.GetPrimaryKeyString());
    }
    ...
    )
    {
        ...
        await State.WriteStateAsync();
        Logger.CacheEntrySet(this.GetPrimaryKeyString(), data.Value.Length);
    }
...
internal static class CacheEntryGrainLoggerExtensions
{
    ...
    private static readonly Action<ILogger, string, string, Exception?> CacheEntryExpiredMessage =
        LoggerMessage.Define<string, string>(
            LogLevel.Debug,
            new(5, nameof(CacheEntryExpired)),
    ...
    public static void CacheEntryExpired(
        this ILogger<CacheEntryGrain> logger,
        string key,
        string reason
    ) =>
    ...
    /// <param name="logger">The logger instance.</param>
    /// <param name="key">The cache entry key.</param>
    public static void CacheEntryRefreshed(
        this ILogger<CacheEntryGrain> logger,
        string key
    ) =>
    ...
    /// <param name="logger">The logger instance.</param>
    /// <param name="key">The cache entry key.</param>
    public static void CacheEntryRemoved(
        this ILogger<CacheEntryGrain> logger,
        string key
    ) =>
    ...
    /// <param name="key">The cache entry key.</param>
    /// <param name="size">The size of the cache entry in bytes.</param>
    public static void CacheEntrySet(
        this ILogger<CacheEntryGrain> logger,
        string key,
        int size
    ) =>
