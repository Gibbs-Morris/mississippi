    ...
    where THub : Hub
{
    ...
    private static Task SendToConnectionAsync(
        ...
        connection.WriteAsync(new SerializedHubMessage(new InvocationMessage(methodName, args)), cancellationToken)
            .AsTask();
    ...
    )
    {
        ...
        Logger.ConnectionRegistered(typeof(THub).Name, connection.ConnectionId, userId);
    }
    ...
    )
    {
        ...
        Logger.ConnectionUnregistered(typeof(THub).Name, connection.ConnectionId);
    }
    ...
    )
    {
        ...
        List<Task> tasks = connections.Select(c => SendToConnectionAsync(c, methodName, args, cancellationToken))
            .ToList();
    ...
    )
    {
        ...
        lock (connectionsLock)
        {
            connections = localConnections.Values.Where(c => !excludedConnectionIds.Contains(c.ConnectionId)).ToList();
        }
    ...
    )
    {
        ...
        lock (connectionsLock)
        {
            connections = localConnections.Values.Where(c => c.UserIdentifier == userId).ToList();
        }
...
internal static class OrleansHubLifetimeManagerLoggerExtensions
{
    private static readonly Action<ILogger, string, string, string?, Exception?> ConnectionRegisteredMessage =
        LoggerMessage.Define<string, string, string?>(
            LogLevel.Debug,
            new(1, nameof(ConnectionRegistered)),
    ...
    private static readonly Action<ILogger, string, string, Exception?> ConnectionUnregisteredMessage =
        LoggerMessage.Define<string, string>(
            LogLevel.Debug,
            new(2, nameof(ConnectionUnregistered)),
    ...
    public static void ConnectionUnregistered(
        this ILogger logger,
        string hubName,
        string connectionId
    ) =>
