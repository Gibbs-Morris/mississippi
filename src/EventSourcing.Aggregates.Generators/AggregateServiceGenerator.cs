...
";
    ...
    /// <inheritdoc />
    public async Task<OperationResult> {command.MethodName}Async(
        string entityId,
        {command.FullTypeName} command,
        CancellationToken cancellationToken = default)
    {{
        IGenericAggregateGrain<{aggregate.FullTypeName}> grain = AggregateGrainFactory.GetGenericAggregate<{aggregate.FullTypeName}>(entityId);
        return await grain.ExecuteAsync(command, cancellationToken);
    }}");
        }
...
/// </summary>
[GeneratedCode(""Mississippi.EventSourcing.Aggregates.Generators"", ""1.0.0"")]
{accessModifier} sealed partial class {aggregate.ServiceName}Service : I{aggregate.ServiceName}Service
{{
    /// <summary>
    ...
    private IAggregateGrainFactory AggregateGrainFactory {{ get; }}{methods}
}}
";
    }
