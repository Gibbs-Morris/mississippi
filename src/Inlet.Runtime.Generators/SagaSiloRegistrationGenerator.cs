...
public sealed class SagaSiloRegistrationGenerator : IIncrementalGenerator
{
    ...
    private const string ISagaBuilderTypeFullName = "Mississippi.DomainModeling.Abstractions.Builders.ISagaBuilder";
    ...
    )
    {
        ...
        sb.OpenBrace();
        sb.AppendSummary($"Adds the {saga.SagaName} saga registrations to the saga builder.");
        sb.AppendLine("/// <param name=\"sagas\">The saga builder.</param>");
        sb.AppendLine("/// <returns>The saga builder for chaining.</returns>");
        sb.AppendLine($"public static ISagaBuilder Add{saga.SagaName}Saga(this ISagaBuilder sagas)");
        sb.OpenBrace();
        sb.AppendLine("ArgumentNullException.ThrowIfNull(sagas);");

        // Start fluent chain
        sb.AppendLine("return sagas");
        sb.IncreaseIndent();
        sb.AppendLine($".AddSagaOrchestration<{saga.SagaStateTypeName}, {saga.InputTypeName}>()");
        foreach (StepInfo step in saga.Steps)
        {
            sb.AppendLine($".AddSagaStep<{step.StepTypeName}>()");
        }

        if (saga.Steps.Count > 0)
        {
            sb.AppendLine($".AddSagaStepInfo<{saga.SagaStateTypeName}>(new SagaStepInfo[]");
            sb.OpenBrace();
            foreach (StepInfo step in saga.Steps)
            {
                sb.AppendLine(
        ...
        {
            sb.AppendLine(
                $".AddReducer<{reducer.EventTypeName}, {saga.SagaStateTypeName}, {reducer.ReducerTypeName}>()");
        }

        sb.AppendLine($".AddSnapshotStateConverter<{saga.SagaStateTypeName}>();");
        sb.DecreaseIndent();
        sb.CloseBrace();
        sb.CloseBrace();
        return sb.ToString();
    ...
    private static bool HasRegistrationDependencies(
        ...
        compilation.GetTypeByMetadataName(ISagaBuilderTypeFullName) is not null;
