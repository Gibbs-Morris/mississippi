...
public sealed class ProjectionSiloRegistrationGenerator : IIncrementalGenerator
{
    ...
    private const string IProjectionBuilderTypeFullName =
        "Mississippi.DomainModeling.Abstractions.Builders.IProjectionBuilder";
    ...
    )
    {
        ...
        sb.OpenBrace();
        sb.AppendSummary($"Adds the {projection.Model.ProjectionName} projection services to the projection builder.");
        sb.AppendLine("/// <param name=\"projections\">The projection builder.</param>");
        sb.AppendLine("/// <returns>The projection builder for chaining.</returns>");
        sb.AppendLine($"public static IProjectionBuilder Add{projection.Model.ProjectionName}Projection(");
        sb.IncreaseIndent();
        sb.AppendLine("this IProjectionBuilder projections");
        sb.DecreaseIndent();
        sb.AppendLine(")");
        sb.OpenBrace();

        // Start fluent chain
        sb.AppendLine("return projections");
        sb.IncreaseIndent();

        // Add projection infrastructure
        sb.AppendLine($".AddProjection<{projection.Model.TypeName}>()");

        // Register reducers
        foreach (ReducerInfo reducer in projection.Reducers)
        {
            sb.AppendLine($".AddReducer<{reducer.EventTypeName}, {projection.Model.TypeName}, {reducer.TypeName}>()");
        }

        // Add snapshot state converter (terminal call with semicolon)
        sb.AppendLine($".AddSnapshotStateConverter<{projection.Model.TypeName}>();");
        sb.DecreaseIndent();
        sb.CloseBrace();
        sb.CloseBrace();
        return sb.ToString();
    ...
    private static bool HasRegistrationDependencies(
        ...
        compilation.GetTypeByMetadataName(IProjectionBuilderTypeFullName) is not null;
