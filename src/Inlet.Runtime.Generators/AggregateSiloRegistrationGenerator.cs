...
public sealed class AggregateSiloRegistrationGenerator : IIncrementalGenerator
{
    ...
    private const string CommandHandlerBaseFullName = "Mississippi.DomainModeling.Abstractions.CommandHandlerBase`2";

    private const string EventEffectBaseFullName = "Mississippi.DomainModeling.Abstractions.EventEffectBase`2";

    private const string EventReducerBaseFullName = "Mississippi.Tributary.Abstractions.EventReducerBase`2";
    ...
    private const string IAggregateBuilderTypeFullName =
        "Mississippi.DomainModeling.Abstractions.Builders.IAggregateBuilder";

    private const string SimpleEventEffectBaseFullName =
        "Mississippi.DomainModeling.Abstractions.SimpleEventEffectBase`2";
    ...
    )
    {
        ...
        sb.AppendUsing("Mississippi.DomainModeling.Abstractions.Builders");
        sb.AppendUsing("Mississippi.DomainModeling.Runtime.Builders");
        ...
        sb.OpenBrace();
        sb.AppendSummary($"Adds the {aggregate.Model.AggregateName} aggregate services to the aggregate builder.");
        sb.AppendLine("/// <param name=\"aggregates\">The aggregate builder.</param>");
        sb.AppendLine("/// <returns>The aggregate builder for chaining.</returns>");
        sb.AppendLine($"public static IAggregateBuilder Add{aggregate.Model.AggregateName}Aggregate(");
        sb.IncreaseIndent();
        sb.AppendLine("this IAggregateBuilder aggregates");
        sb.DecreaseIndent();
        sb.AppendLine(")");
        sb.OpenBrace();

        // Start fluent chain
        sb.AppendLine("return aggregates");
        sb.IncreaseIndent();

        // Add aggregate infrastructure
        sb.AppendLine($".AddAggregate<{aggregate.Model.TypeName}>()");
        ...
        {
            sb.AppendLine($".AddEventType<{reducer.EventTypeName}>()");
        }
        ...
        {
            sb.AppendLine(
                $".AddCommandHandler<{handler.CommandTypeName}, {aggregate.Model.TypeName}, {handler.TypeName}>()");
        }

        // Register reducers for state computation
        foreach (ReducerInfo reducer in aggregate.Reducers)
        {
            sb.AppendLine($".AddReducer<{reducer.EventTypeName}, {aggregate.Model.TypeName}, {reducer.TypeName}>()");
        }

        // Register event effects if present
        foreach (EffectInfo effect in aggregate.Effects)
        {
            sb.AppendLine($".AddEventEffect<{effect.TypeName}, {aggregate.Model.TypeName}>()");
        }
        ...
        {
            sb.AppendLine(
                $".AddFireAndForgetEventEffect<{effect.TypeName}, {effect.EventTypeName}, {aggregate.Model.TypeName}>()");
        }

        // Add snapshot state converter (terminal call with semicolon)
        sb.AppendLine($".AddSnapshotStateConverter<{aggregate.Model.TypeName}>();");
        sb.DecreaseIndent();
        sb.CloseBrace();
        sb.CloseBrace();
        return sb.ToString();
    ...
    private static bool HasRegistrationDependencies(
        ...
        compilation.GetTypeByMetadataName(IAggregateBuilderTypeFullName) is not null;
    ...
    )
    {
        ...
        if (ns != "Mississippi.DomainModeling.Abstractions")
        {
            return false;
        }
    ...
    )
    {
        ...
        return constructedFrom is not null &&
               ...
               (constructedFrom.ContainingNamespace.ToDisplayString() == "Mississippi.Tributary.Abstractions");
    }
