...
public sealed class DelegateEventReducer<TEvent, TProjection> : EventReducerBase<TEvent, TProjection>
{
    /// <summary>
    ///     Initializes a new instance of the <see cref="DelegateEventReducer{TEvent,TProjection}" /> class.
    /// </summary>
    ...
    public DelegateEventReducer(
        Func<TProjection, TEvent, TProjection> reduce,
        ILogger<DelegateEventReducer<TEvent, TProjection>>? logger = null
    )
    {
        ReduceDelegate = reduce ?? throw new ArgumentNullException(nameof(reduce));
        Logger = logger ?? NullLogger<DelegateEventReducer<TEvent, TProjection>>.Instance;
    }

    private ILogger<DelegateEventReducer<TEvent, TProjection>> Logger { get; }
