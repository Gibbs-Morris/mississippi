...
///     </para>
///     <para>
///         For isolated unit tests, apply a reducer directly to an initial state and assert on the
///         returned projection, or use the test extension helpers that compare the produced state to
...
///     </para>
///     <para>
///         For event replay scenarios, register the relevant reducers, create a scenario from the
///         harness, establish the starting state with <c>Given</c>, apply the event under test with
///         <c>When</c>, and verify the final projection with the fluent assertion helpers.
///     </para>
/// </remarks>
    ...
    where TProjection : new()
{
    ...
    /// </summary>
    public ReducerTestHarness() => InitialState = new();

    /// <summary>
    ///     Gets the initial state for scenarios.
    /// </summary>
    internal TProjection InitialState { get; private set; }
    ...
    /// </exception>
    public TProjection ApplyEvent<TEvent>(
        TEvent eventData
    )
        where TEvent : class
    {
    ...
    /// </exception>
    public TProjection ApplyEvents(
    ...
    public ProjectionScenario<TProjection> CreateScenario() => new(reducers, InitialState);
    ...
    public ReducerTestHarness<TProjection> WithInitialState(
        TProjection state
    )
    {
    ...
    public ReducerTestHarness<TProjection> WithReducer(
        IEventReducer<TProjection> reducer
    )
    {
