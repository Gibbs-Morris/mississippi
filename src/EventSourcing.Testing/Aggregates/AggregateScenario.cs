    ...
    where TAggregate : new()
{
    ...
    private TAggregate currentState;
    ...
    /// </summary>
    public TAggregate State => currentState;
    ...
    public AggregateScenario<TAggregate> Given(object eventData)
    {
    ...
    public AggregateScenario<TAggregate> When(object command)
    {
        ...
        (IEnumerable<object> events, bool success, string? errorCode, string? errorMessage) = ExecuteCommand(currentState, command);
    ...
    /// <returns>This scenario for fluent chaining.</returns>
    public AggregateScenario<TAggregate> ThenEmits<TEvent>(Action<TEvent>? assertion = null)
        where TEvent : class
    {
    ...
    /// <returns>This scenario for fluent chaining.</returns>
    public AggregateScenario<TAggregate> ThenEmitsEvents(params Action<object>[] assertions)
    {
    ...
    /// <returns>This scenario for fluent chaining.</returns>
    public AggregateScenario<TAggregate> ThenState(Action<TAggregate> assertion)
    {
    ...
    /// <returns>This scenario for fluent chaining.</returns>
    [CustomAssertion]
    public AggregateScenario<TAggregate> ThenFails(string expectedErrorCode)
    {
        ArgumentNullException.ThrowIfNull(expectedErrorCode);

        whenCommand.Should().NotBeNull("When() must be called before ThenFails()");
        lastCommandSucceeded.Should().BeFalse("Command should have failed but succeeded with {0} events", emittedEvents.Count);
    ...
    /// <returns>This scenario for fluent chaining.</returns>
    [CustomAssertion]
    public AggregateScenario<TAggregate> ThenFails(string expectedErrorCode, string expectedMessage)
    {
        ArgumentNullException.ThrowIfNull(expectedErrorCode);
    ...
    private TAggregate ApplyEvent(TAggregate state, object evt)
    {
    ...
    private static bool CanHandle(IEventReducer<TAggregate> reducer, object evt)
    {
    ...
    private static TAggregate ApplyWithReducer(IEventReducer<TAggregate> reducer, TAggregate state, object evt)
    {
        Type reducerType = reducer.GetType();
        Type eventType = evt.GetType();
    ...
    private (IEnumerable<object> Events, bool Success, string? ErrorCode, string? ErrorMessage) ExecuteCommand(TAggregate state, object command)
    {
    ...
    private static bool CanHandleCommand(ICommandHandler<TAggregate> handler, object command)
    {
    ...
    private static (IEnumerable<object> Events, bool Success, string? ErrorCode, string? ErrorMessage) ExecuteWithHandler(ICommandHandler<TAggregate> handler, TAggregate state, object command)
    {
        Type handlerType = handler.GetType();
        Type commandType = command.GetType();
        ...
        foreach (Type iface in handlerType.GetInterfaces())
        {
            if (iface.IsGenericType &&
                ...
                iface.GetGenericArguments()[0] == commandType)
            {
                System.Reflection.MethodInfo? method = iface.GetMethod("Handle");
                object result = method!.Invoke(handler, [command, state])!;

                // Handle OperationResult<IReadOnlyList<object>> return type
                Type resultType = result.GetType();
                System.Reflection.PropertyInfo? successProp = resultType.GetProperty("Success");
                System.Reflection.PropertyInfo? valueProp = resultType.GetProperty("Value");
                System.Reflection.PropertyInfo? errorCodeProp = resultType.GetProperty("ErrorCode");
                System.Reflection.PropertyInfo? errorMessageProp = resultType.GetProperty("ErrorMessage");
