...
///         <list type="bullet">
///             <item>
///                 <see cref="IUxClientGrain" /> - Tracks individual connections and their hosting servers.
///             </item>
///             <item>
///                 <see cref="IUxGroupGrain" /> - Manages group membership and broadcasts to groups.
///             </item>
///             <item>
///                 <see cref="IUxServerDirectoryGrain" /> - Tracks active servers for failure detection.
///             </item>
///         </list>
///     </para>
///     <para>
...
///     </para>
/// </remarks>
internal sealed class OrleansHubLifetimeManager<THub>
    ...
    where THub : Hub
{
    ...
    )
    {
        ...
        IUxGroupGrain groupGrain = GetGroupGrain(groupName);
    ...
    public void Dispose()
    {
        ...
        IUxServerDirectoryGrain directoryGrain = GetServerDirectoryGrain();
    ...
    )
    {
        ...
        await EnsureStreamSetupAsync();
        connections[connection.ConnectionId] = connection;
        IUxClientGrain clientGrain = GetClientGrain(connection.ConnectionId);
    ...
    )
    {
        ...
        await EnsureStreamSetupAsync(cancellationToken);
        AllMessage message = new()
        {
            MethodName = methodName,
            Args = args ?? [],
        };
        await allStream!.OnNextAsync(message);
    }
    ...
    )
    {
        ...
        {
            await SendLocalAsync(connection, methodName, args);
            return;
        }

        // Route via client grain for remote connections
        IUxClientGrain clientGrain = GetClientGrain(connectionId);
    ...
    )
    {
        ...
        IUxGroupGrain groupGrain = GetGroupGrain(groupName);
        ImmutableHashSet<string> members = await groupGrain.GetConnectionsAsync();
    ...
    )
    {
        ...
        await SendGroupAsync(userGroupName, methodName, args, cancellationToken);
    }
    ...
    private IUxClientGrain GetClientGrain(
        string connectionId
    ) =>
        ClusterClient.GetGrain<IUxClientGrain>($"{hubName}:{connectionId}");

    private IUxGroupGrain GetGroupGrain(
        string groupName
    ) =>
        ClusterClient.GetGrain<IUxGroupGrain>($"{hubName}:{groupName}");

    private IUxServerDirectoryGrain GetServerDirectoryGrain() =>
        ClusterClient.GetGrain<IUxServerDirectoryGrain>("default");
    ...
    )
    {
        foreach (HubConnectionContext connection in connections.Values)
        {
            ...
            if (!isExcluded)
            {
                await SendLocalAsync(connection, message.MethodName, message.Args);
            }
        }
    }
    ...
    )
    {
        ...
        await connection.WriteAsync(invocation);
    }
}