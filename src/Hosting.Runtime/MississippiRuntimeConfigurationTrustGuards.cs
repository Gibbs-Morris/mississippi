...
internal static class MississippiRuntimeConfigurationTrustGuards
{
    ...
    )
    {
        ...
        bool hasExternalEndpoint = endpoints.Any(endpoint => ClassifyHost(endpoint.Host) == MississippiRuntimeEndpointHostClass.External);
        bool hasLocalEndpoint = endpoints.Any(endpoint => ClassifyHost(endpoint.Host) != MississippiRuntimeEndpointHostClass.External);
    ...
    )
    {
        ...
        if (!segments.TryGetValue("AccountName", out string? accountName) ||
        ...
        {
            Uri derivedEndpoint = new($"{defaultProtocol}://{accountName}.{endpointPrefix}.{endpointSuffix}", UriKind.Absolute);
    ...
    private sealed class MississippiRuntimeTrustPolicy
    {
        ...
        )
        {
            ...
            return new(
                ...
                new HashSet<string>(schemes, StringComparer.OrdinalIgnoreCase),
                new HashSet<string>(ReadArray(section.GetSection("AllowedExternalHosts")), StringComparer.OrdinalIgnoreCase));
        }
