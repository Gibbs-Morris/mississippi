...
public sealed class AggregateSiloRegistrationGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        sb.OpenBrace();
        sb.AppendSummary(
            $"Adds the {aggregate.Model.AggregateName} aggregate services to the Mississippi silo builder.");
        ...
        sb.AppendLine($"public static IMississippiSiloBuilder Add{aggregate.Model.AggregateName}Aggregate(");
        sb.IncreaseIndent();
        ...
        sb.AppendLine("builder.AddAggregateSupport();");
        sb.AppendLine();
        ...
        {
            sb.AppendLine($"builder.AddEventType<{reducer.EventTypeName}>();");
        }
        ...
        {
            sb.AppendLine(
                $"builder.AddCommandHandler<{handler.CommandTypeName}, {aggregate.Model.TypeName}, {handler.TypeName}>();");
        }
        ...
        {
            sb.AppendLine(
                $"builder.AddReducer<{reducer.EventTypeName}, {aggregate.Model.TypeName}, {reducer.TypeName}>();");
        }
        ...
        if (aggregate.Effects.Count > 0)
        {
            ...
            {
                sb.AppendLine($"builder.AddEventEffect<{effect.TypeName}, {aggregate.Model.TypeName}>();");
            }
        ...
        if (aggregate.FireAndForgetEffects.Count > 0)
        {
            ...
            {
                sb.AppendLine(
                    $"builder.AddFireAndForgetEventEffect<{effect.TypeName}, {effect.EventTypeName}, {aggregate.Model.TypeName}>();");
            }
        ...
        sb.AppendLine($"builder.AddSnapshotStateConverter<{aggregate.Model.TypeName}>();");
