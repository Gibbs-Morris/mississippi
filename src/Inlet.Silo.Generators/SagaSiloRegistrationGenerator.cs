...
public sealed class SagaSiloRegistrationGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        sb.AppendLine("/// <returns>The updated builder.</returns>");
        sb.AppendLine(
            $"public static IMississippiSiloBuilder Add{saga.SagaName}Saga(this IMississippiSiloBuilder builder)");
        sb.OpenBrace();
        sb.AppendLine("ArgumentNullException.ThrowIfNull(builder);");
        sb.AppendLine($"builder.AddSagaOrchestration<{saga.SagaStateTypeName}, {saga.InputTypeName}>();");
        sb.AppendLine($"builder.AddSnapshotStateConverter<{saga.SagaStateTypeName}>();");
        foreach (StepInfo step in saga.Steps)
        {
            sb.AppendLine($"builder.ConfigureServices(services => services.AddTransient<{step.StepTypeName}>());");
        }

        if (saga.Steps.Count > 0)
        {
            sb.AppendLine($"builder.AddSagaStepInfo<{saga.SagaStateTypeName}>(new SagaStepInfo[]");
            sb.OpenBrace();
            foreach (StepInfo step in saga.Steps)
            {
                sb.AppendLine(
        ...
        {
            sb.AppendLine(
                $"builder.AddReducer<{reducer.EventTypeName}, {saga.SagaStateTypeName}, {reducer.ReducerTypeName}>();");
        }
