...
internal static partial class RootReducerLoggerExtensions
{
    ...
    /// <param name="projectionType">The projection type name.</param>
    /// <param name="eventType">The event type name.</param>
    [LoggerMessage(3, LogLevel.Debug, "No reducer matched for projection {ProjectionType} and event {EventType}; returning unchanged state")]
    ...
    /// <param name="projectionType">The projection type name.</param>
    /// <param name="eventType">The event type name.</param>
    [LoggerMessage(4, LogLevel.Error, "Reducer {ReducerType} returned the incoming projection instance for projection {ProjectionType} when handling event {EventType}. Reducers must return a new instance (use copy/with expressions). ")]
