...
public sealed class CommandClientActionsGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        context.RegisterSourceOutput(
            ...
            ) =>
            {
                ...
                // Generate per-aggregate SetEntityIdAction
                HashSet<string> generatedAggregates = new();
                foreach (CommandInfo command in commands)
                {
                    string? aggregateName = NamingConventions.GetAggregateNameFromNamespace(command.Model.Namespace);
                    if (aggregateName is not null && generatedAggregates.Add(aggregateName))
                    {
                        GenerateSetEntityIdAction(spc, command.ActionsNamespace);
                    }
                }
            });
    }
    ...
    /// <summary>
    ///     Generates the SetEntityIdAction for an aggregate.
    /// </summary>
    private static void GenerateSetEntityIdAction(
        SourceProductionContext context,
    ...
    )
    {
        ...
        sb.AppendFileScopedNamespace(actionsNamespace);
        sb.AppendLine();
        sb.AppendSummary("Action dispatched to set the current entity ID for targeting commands.");
        sb.AppendLine("/// <remarks>");
        sb.AppendLine("///     This action updates the aggregate command state with the entity ID");
        sb.AppendLine("///     that subsequent commands should target.");
        sb.AppendLine("/// </remarks>");
        sb.AppendLine("/// <param name=\"EntityId\">The entity ID to set as current.</param>");
        sb.AppendGeneratedCodeAttribute("CommandClientActionsGenerator");
        sb.AppendLine("internal sealed record SetEntityIdAction(string EntityId) : IAction;");
        context.AddSource("SetEntityIdAction.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
    }
