    ...
    where TSaga : class, ISagaState
{
    ...
    )
    {
        ...
        {
            SagaRecoveryPlanDisposition.NoAction or SagaRecoveryPlanDisposition.Terminal
                or SagaRecoveryPlanDisposition.WorkflowMismatch => OperationResult.Ok<IReadOnlyList<object>>([]),
            SagaRecoveryPlanDisposition.CompleteSaga => OperationResult.Ok<IReadOnlyList<object>>(
            [
                new SagaCompleted
                {
                    CompletedAt = now,
            ...
            SagaRecoveryPlanDisposition.CompensateSaga => OperationResult.Ok<IReadOnlyList<object>>(
            [
                new SagaCompensated
                {
                    CompletedAt = now,
            ...
            SagaRecoveryPlanDisposition.Blocked => BuildBlockedResult(command, now),
            SagaRecoveryPlanDisposition.ExecuteStep => BuildExecutionStartedResult(command, state, now),
            var _ => OperationResult.Fail<IReadOnlyList<object>>(
