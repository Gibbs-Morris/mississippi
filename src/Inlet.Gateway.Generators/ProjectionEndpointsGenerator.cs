...
public sealed class ProjectionEndpointsGenerator : IIncrementalGenerator
{
    ...
    )
    {
                     ...
                     .Where(enumInfo => !generatedNestedTypes.Contains(enumInfo.DtoName)))
        {
            ...
            context.AddSource($"{enumInfo.DtoName}.g.cs", SourceText.From(enumDtoSource, Encoding.UTF8));
            string enumMapperSource = GenerateEnumMapper(
                enumInfo.EnumType,
                enumInfo.DtoName,
                projection.OutputNamespace);
            context.AddSource(
                $"{enumInfo.DtoName}{MapperSuffix}.g.cs",
    ...
    )
    {
        ...
        if (projection.Authorization.HasAnyAuthorizationMetadata)
        {
        ...
        GeneratedApiAuthorizationAnalysis.AppendAuthorizationAttributes(sb, projection.Authorization);
    ...
    /// <summary>
    ///     Generates a mapper for an enum type.
    /// </summary>
    private static string GenerateEnumMapper(
        INamedTypeSymbol sourceType,
        string dtoName,
        string outputNamespace
    )
    {
        ...
        sb.AppendLine($"public {dtoName} Map({sourceTypeName} source)");
        sb.OpenBrace();
        sb.AppendLine($"return ({dtoName})source;");
        sb.CloseBrace();
        sb.CloseBrace();
        return sb.ToString();
    }
    ...
    )
    {
        ...
        // Register nested type mappers (required for enumerable mappers to resolve)
