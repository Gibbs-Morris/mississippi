...
internal sealed class BlobEnvelopeCodec : IBlobEnvelopeCodec
{
    ...
    )
    {
        ...
        PayloadSerializerResolver = payloadSerializerResolver ??
                                    throw new ArgumentNullException(nameof(payloadSerializerResolver));
        Options = options?.Value ?? throw new ArgumentNullException(nameof(options));
        TimeProvider = timeProvider ?? throw new ArgumentNullException(nameof(timeProvider));
        KnownPayloadSerializerIds =
            serializationProviders?.Select(SnapshotPayloadSerializerResolver.GetSerializerId)
    ...
    )
    {
        try
        {
            using MemoryStream source = new(storedPayloadBytes.ToArray(), false);
    ...
    private static SnapshotBlobCompression ParseCompression(
        ...
        compressionAlgorithm switch
        {
            ...
            var _ => throw CreateUnreadableException(
                snapshotKey,
    ...
    )
    {
        try
        {
            StoredSnapshotBlobHeader? header =
                JsonSerializer.Deserialize<StoredSnapshotBlobHeader>(headerBytes, HeaderSerializerOptions);
            return header ??
                   throw CreateUnreadableException(
                       snapshotKey,
