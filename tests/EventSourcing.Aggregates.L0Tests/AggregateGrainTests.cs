...
public class AggregateGrainTests
{
    ...
    )
    {
        ...
        brookGrainFactoryMock ??= new Mock<IBrookGrainFactory>();
        serializationProviderMock ??= new Mock<ISerializationProvider>();
        rootReducerMock ??= new Mock<IRootReducer<AggregateGrainTestState>>();
        loggerMock ??= new Mock<ILogger>();

        return new TestableAggregateGrain(
    ...
    public async Task OnActivateAsyncHydratesStateFromBrookEvents()
    {
        ...
        brookGrainFactoryMock.Setup(f => f.GetBrookReaderGrain(It.IsAny<BrookKey>()))
            .Returns(readerGrainMock.Object);

        serializationProviderMock.Setup(s => s.Read<object>(It.IsAny<ReadOnlyMemory<byte>>()))
            .Returns(event1);
        ...
        rootReducerMock.Verify(r => r.Reduce(It.IsAny<AggregateGrainTestState>(), It.IsAny<object>()), Times.Exactly(2));
    }
    ...
    public async Task ExecuteAsyncPersistsEventsAndUpdatesState()
    {
        ...
        brookGrainFactoryMock.Setup(f => f.GetBrookReaderGrain(It.IsAny<BrookKey>()))
            .Returns(readerGrainMock.Object);
        brookGrainFactoryMock.Setup(f => f.GetBrookWriterGrain(It.IsAny<BrookKey>()))
            .Returns(writerGrainMock.Object);

        Mock<ISerializationProvider> serializationProviderMock = new();
        serializationProviderMock.Setup(s => s.Write(It.IsAny<AggregateGrainTestEvent>()))
    ...
    /// <summary>
    ///     ResolveEventType should return null when registry is not registered.
    /// </summary>
    /// <returns>A task representing the asynchronous operation.</returns>
    [Fact]
    public async Task ResolveEventTypeReturnsNullWhenRegistryNotRegistered()
    {
        TestableAggregateGrain grain = await CreateActivatedGrainAsync();
    ...
    {
        Mock<IEventTypeRegistry> registryMock = new();
        registryMock.Setup(r => r.ResolveType("TEST.AGGREGATES.TESTEVENTV1"))
            .Returns(typeof(AggregateGrainTestEvent));
