...
using Mississippi.Aqueduct.Runtime.L0Tests.Infrastructure;
...
{
    /// <summary>
    ///     Tests that connect sets the server ID.
    /// </summary>
    ...
    {
        // Arrange
        ISignalRClientGrain grain = TestClusterAccess.Cluster.Client.GetGrain<ISignalRClientGrain>("TestHub:conn-1");

        // Act
        await grain.ConnectAsync("TestHub", "server-1");
    ...
    /// <summary>
    ///     Tests that connect can update server.
    /// </summary>
    /// <returns>A <see cref="Task" /> representing the asynchronous unit test.</returns>
    ...
    {
        // Arrange
        ISignalRClientGrain grain = TestClusterAccess.Cluster.Client.GetGrain<ISignalRClientGrain>("TestHub:conn-3");
        await grain.ConnectAsync("TestHub", "server-1");

        // Act
        await grain.ConnectAsync("TestHub", "server-2");
        string? serverId = await grain.GetServerIdAsync();

        // Assert
        Assert.Equal("server-2", serverId);
    }
    ...
    {
        // Arrange
        ISignalRClientGrain grain = TestClusterAccess.Cluster.Client.GetGrain<ISignalRClientGrain>("TestHub:conn-4");
        await grain.ConnectAsync("TestHub", "server-1");
    ...
    /// <summary>
    ///     Tests that disconnect clears the server ID.
    /// </summary>
    /// <returns>A <see cref="Task" /> representing the asynchronous unit test.</returns>
    ...
    {
        // Arrange
        ISignalRClientGrain grain = TestClusterAccess.Cluster.Client.GetGrain<ISignalRClientGrain>("TestHub:conn-2");
        await grain.ConnectAsync("TestHub", "server-1");
    ...
    {
        // Arrange
        ISignalRClientGrain grain = TestClusterAccess.Cluster.Client.GetGrain<ISignalRClientGrain>("TestHub:new-grain");
