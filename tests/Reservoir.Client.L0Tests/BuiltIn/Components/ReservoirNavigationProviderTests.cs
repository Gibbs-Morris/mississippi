...
public sealed class ReservoirNavigationProviderTests : IDisposable
{
    ...
    /// <summary>
    ///     The constructor should reject a null navigation manager.
    /// </summary>
    [Fact]
    public void ConstructorRejectsNullNavigationManager()
    {
        ArgumentNullException exception =
            Assert.Throws<ArgumentNullException>(() => new ReservoirNavigationProvider(null!, store).Dispose());
        Assert.Equal("navigationManager", exception.ParamName);
    }

    /// <summary>
    ///     The constructor should reject a null store.
    /// </summary>
    [Fact]
    public void ConstructorRejectsNullStore()
    {
        ArgumentNullException exception = Assert.Throws<ArgumentNullException>(() =>
            new ReservoirNavigationProvider(navigationManager, null!).Dispose());
