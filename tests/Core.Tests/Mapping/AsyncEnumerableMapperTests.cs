...
public class AsyncEnumerableMapperTests
{
    ...
    public async Task MapsAsyncCollectionCorrectlyAsync()
    {
        ...
        // Act – enumerate without
        List<string> result = new();
        await foreach (string item in asyncEnumerableMapper.Map(input))
        {
            result.Add(item);
        }
        ...
        await Assert.ThrowsAsync<ArgumentNullException>(async () =>
            {
                ...
                List<string> sink = [];
                await foreach (string item in asyncEnumerableMapper.Map(null!))
                {
                    sink.Add(item);
                }
            })
    ...
    /// <summary>
    ///     Tests that an asynchronous collection with null elements is mapped correctly,
    ///     with null elements represented as "null".
    /// </summary>
    /// <returns>A task representing the asynchronous operation.</returns>
    [Fact]
    public async Task MapsAsyncCollectionWithNullElementsAsync()
    {
        ...
        // Act – materialise the async stream without System.Linq.Async
        List<string> result = new();
        await foreach (string item in asyncEnumerableMapper.Map(input))
        {
            result.Add(item);
