...
public sealed class SnapshotBlobRepositoryTests
{
    ...
    internal static BlobNameStrategy CreateNameStrategy() => new(Options.Create(new SnapshotBlobStorageOptions()));
    ...
    )
    {
        ...
        return new(nameStrategy, resolver, [serializer], Options.Create(options), new FakeTimeProvider());
    }
    ...
    public async Task DeleteAllAsyncShouldDeleteOnlyTheTargetStreamWithoutDownloadingBodies()
    {
        ...
        await repository.WriteAsync(
            new(targetStream, 9),
            CreateSnapshotEnvelope([9], targetStream.ReducersHash),
            CancellationToken.None);
        await repository.WriteAsync(
            new(targetStream, 10),
            CreateSnapshotEnvelope([10], targetStream.ReducersHash),
            CancellationToken.None);
        await repository.WriteAsync(
            new(otherStream, 77),
            CreateSnapshotEnvelope([77], otherStream.ReducersHash),
            CancellationToken.None);
        await repository.DeleteAllAsync(targetStream, CancellationToken.None);
        Assert.Empty(operations.DownloadedBlobNames);
        Assert.Collection(operations.DeletedBlobNames, static _ => { }, static _ => { });
        Assert.Single(operations.Blobs);
        string remainingBlobName = Assert.Single(operations.Blobs.Keys);
        Assert.Contains("v00000000000000000077.snapshot", remainingBlobName, StringComparison.Ordinal);
    }
    ...
    public async Task ListVersionsAsyncShouldReturnOnlyVersionsForTheRequestedStream()
    {
        ...
        operations.Pages.Add(
            ...
            [
                ...
                nameStrategy.GetBlobName(new(new("brook-a", "projection-a", "entity-43", "reducers-v1"), 999)),
                "not-a-snapshot-name",
            ]));
        SnapshotBlobRepository repository = CreateRepository(operations, nameStrategy);
    ...
    public async Task PruneAsyncShouldRetainModulusMatchesAndLatestWithoutDownloadingBodies()
    {
        ...
        {
            await repository.WriteAsync(
                new(streamKey, version),
                CreateSnapshotEnvelope([(byte)version], streamKey.ReducersHash),
                CancellationToken.None);
        }
        ...
        Assert.DoesNotContain(
            operations.Blobs.Keys,
            blobName => blobName.Contains("v00000000000000000001.snapshot", StringComparison.Ordinal));
        Assert.DoesNotContain(
            operations.Blobs.Keys,
            blobName => blobName.Contains("v00000000000000000003.snapshot", StringComparison.Ordinal));
        Assert.Contains(
            operations.Blobs.Keys,
            blobName => blobName.Contains("v00000000000000000002.snapshot", StringComparison.Ordinal));
        Assert.Contains(
            operations.Blobs.Keys,
            blobName => blobName.Contains("v00000000000000000004.snapshot", StringComparison.Ordinal));
        Assert.Contains(
            operations.Blobs.Keys,
            blobName => blobName.Contains("v00000000000000000005.snapshot", StringComparison.Ordinal));
    }
    ...
    public async Task ReadLatestAsyncShouldDownloadOnlyTheSelectedLatestBlob()
    {
        ...
        await repository.WriteAsync(
            new(streamKey, 9),
            CreateSnapshotEnvelope([9], streamKey.ReducersHash),
            CancellationToken.None);
        await repository.WriteAsync(
            new(streamKey, 10),
            CreateSnapshotEnvelope([10], streamKey.ReducersHash),
            CancellationToken.None);
        await repository.WriteAsync(
            new(streamKey, 100),
            CreateSnapshotEnvelope([100], streamKey.ReducersHash),
            CancellationToken.None);
        ...
        Assert.EndsWith("v00000000000000000100.snapshot", downloadedBlobName, StringComparison.Ordinal);
    }
    ...
    {
        SnapshotStreamKey streamKey = new("brook-a", "projection-a", "entity-42", "reducers-v1");
        SnapshotBlobRepository repository = CreateRepository(new());
    ...
    )
    {
        ...
        SnapshotKey snapshotKey = new(new("brook-a", "projection-a", "entity-42", "reducers-v1"), 12);
        SnapshotEnvelope snapshot = CreateSnapshotEnvelope(
            [1, 2, 3, 4],
            snapshotKey.Stream.ReducersHash,
    ...
    public async Task WriteIfAbsentAsyncShouldThrowDuplicateVersionConflictWhenBlobAlreadyExists()
    {
        ...
        SnapshotBlobDuplicateVersionException exception =
            await Assert.ThrowsAsync<SnapshotBlobDuplicateVersionException>(() =>
    ...
    public async Task WriteIfAbsentAsyncShouldUseConditionalCreateAndSucceedWhenBlobIsNew()
    {
        ...
        Assert.EndsWith("v00000000000000000012.snapshot", operations.CreatedBlobNames[0], StringComparison.Ordinal);
    }
}