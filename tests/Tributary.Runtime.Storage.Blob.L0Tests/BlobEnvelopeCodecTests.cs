...
public sealed class BlobEnvelopeCodecTests
{
    ...
    )
    {
        ...
        return new(
            ...
            new(serializationProviders, Options.Create(options)),
    ...
    {
        byte[] payload = Enumerable.Range(0, payloadBytes).Select(index => (byte)(index % 251)).ToArray();
        return new()
        {
    ...
    private static SnapshotKey CreateSnapshotKey() =>
        new(new("brook-a", "projection-a", "entity-42", "reducers-v1"), 12);
    ...
    public void DecodeShouldFailForUnknownPersistedSerializerIdentity()
    {
        ...
        BlobEnvelopeCodec readCodec = CreateCodec(
            ...
            payloadSerializerFormat: "custom-json");
        byte[] storedFrame = writeCodec.Encode(snapshotKey, snapshot);
        SnapshotBlobUnreadableFrameException exception =
            Assert.Throws<SnapshotBlobUnreadableFrameException>(() => readCodec.Decode(snapshotKey, storedFrame));
    ...
    public void DecodeShouldFailWhenCompressedPayloadIsInvalid()
    {
        ...
        byte[] storedFrame = CreateCodec(
            ...
            .Encode(snapshotKey, CreateSnapshotEnvelope(4096));
        ...
        SnapshotBlobUnreadableFrameException exception = Assert.Throws<SnapshotBlobUnreadableFrameException>(() =>
            CreateCodec([new TestSerializationProvider("System.Text.Json")], SnapshotBlobCompression.Gzip)
                .Decode(snapshotKey, storedFrame));
    ...
    {
        SnapshotKey snapshotKey = CreateSnapshotKey();
        byte[] storedFrame = CreateCodec([new TestSerializationProvider("System.Text.Json")])
            .Encode(snapshotKey, CreateSnapshotEnvelope(512));
        byte[] original = Encoding.UTF8.GetBytes("\"off\"");
        byte[] replacement = Encoding.UTF8.GetBytes("\"bad\"");
    ...
    public void DecodeShouldFailWhenHeaderLengthExceedsConfiguredMaximum()
    {
        SnapshotKey snapshotKey = CreateSnapshotKey();
        byte[] storedFrame = CreateCodec([new TestSerializationProvider("System.Text.Json")])
            .Encode(snapshotKey, CreateSnapshotEnvelope(1024));
        SnapshotBlobUnreadableFrameException exception = Assert.Throws<SnapshotBlobUnreadableFrameException>(() =>
            CreateCodec([new TestSerializationProvider("System.Text.Json")], maxHeaderBytes: 32)
                .Decode(snapshotKey, storedFrame));
    ...
    public void DecodeShouldFailWhenUncompressedPayloadLengthDoesNotMatchHeader()
    {
        ...
        ReplaceFirst(
            storedFrame,
            Encoding.UTF8.GetBytes("\"uncompressedPayloadBytes\":512"),
            Encoding.UTF8.GetBytes("\"uncompressedPayloadBytes\":513"));
        SnapshotBlobUnreadableFrameException exception = Assert.Throws<SnapshotBlobUnreadableFrameException>(() =>
            CreateCodec([new TestSerializationProvider("System.Text.Json")]).Decode(snapshotKey, storedFrame));
    ...
    )
    {
        ...
        Assert.Equal(
            "Mississippi.Tributary.Runtime.Storage.Blob.L0Tests.TestSerializationProvider",
            decodedFrame.Header.PayloadSerializerId);
