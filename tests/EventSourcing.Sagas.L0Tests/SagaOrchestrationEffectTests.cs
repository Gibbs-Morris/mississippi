...
public sealed class SagaOrchestrationEffectTests
{
    ...
    public void CanHandleRecognizesSagaEvents()
    {
        ...
        Assert.True(effect.CanHandle(new SagaStartedEvent
        {
            SagaId = Guid.NewGuid(),
            StartedAt = now,
            StepHash = "HASH",
        }));
        Assert.True(effect.CanHandle(new SagaStepCompleted
        {
            StepIndex = 0,
            StepName = "Step",
            CompletedAt = now,
        }));
        Assert.True(effect.CanHandle(new SagaStepFailed
        {
            StepIndex = 0,
            StepName = "Step",
            ErrorCode = "ERR",
        }));
        Assert.True(effect.CanHandle(new SagaCompensating
        {
            FromStepIndex = 0,
        }));
        Assert.True(effect.CanHandle(new SagaStepCompensated
        {
            StepIndex = 0,
            StepName = "Step",
        }));
        Assert.False(effect.CanHandle(new object()));
    }
    ...
    public async Task HandleAsyncReturnsEmptyForUnsupportedEvent()
    {
        using ServiceProvider provider = CreateProvider();
        SagaOrchestrationEffect<TestSagaState> effect = CreateEffect(Array.Empty<SagaStepInfo>(), provider);
        List<object> events = await CollectAsync(effect.HandleAsync(
            new object(),
            new TestSagaState(),
            "saga",
            0,
    ...
    public async Task HandleAsyncCompensatesWhenIndexNegative()
    {
        ...
        FakeTimeProvider timeProvider = new(now);
        using ServiceProvider provider = CreateProvider();
        SagaOrchestrationEffect<TestSagaState> effect = CreateEffect(Array.Empty<SagaStepInfo>(), provider, timeProvider);
        List<object> events = await CollectAsync(effect.HandleAsync(
    ...
    public async Task HandleAsyncTreatsSkippedCompensationAsCompensated()
    {
        ...
        using ServiceProvider provider = CreateProvider(services =>
            services.AddTransient<SagaCompensationSkippedStep>());
        SagaOrchestrationEffect<TestSagaState> effect = CreateEffect(steps, provider);
        List<object> events = await CollectAsync(effect.HandleAsync(
            new SagaCompensating
            {
    ...
    public async Task HandleAsyncFailsWhenCompensationFails()
    {
        ...
        using ServiceProvider provider = CreateProvider(services =>
            services.AddTransient<SagaCompensationFailStep>());
        SagaOrchestrationEffect<TestSagaState> effect = CreateEffect(steps, provider);
        List<object> events = await CollectAsync(effect.HandleAsync(
            new SagaCompensating
            {
    ...
    public async Task HandleAsyncThrowsWhenStepTypeInvalid()
    {
        ...
        using ServiceProvider provider = CreateProvider(services =>
            services.AddTransient<SagaInvalidStep>());
        SagaOrchestrationEffect<TestSagaState> effect = CreateEffect(steps, provider);
        await Assert.ThrowsAsync<InvalidOperationException>(async () =>
        {
            await foreach (object item in effect.HandleAsync(
                ...
                {
                    SagaId = Guid.NewGuid(),
                    StartedAt = new DateTimeOffset(2025, 2, 12, 13, 0, 0, TimeSpan.Zero),
                    StepHash = "HASH",
                },
                new TestSagaState(),
                "saga",
    ...
    public async Task HandleAsyncSkipsWhenNoStepsConfigured()
    {
        ...
        List<object> events = await CollectAsync(effect.HandleAsync(
            ...
            {
                SagaId = Guid.NewGuid(),
                StartedAt = new DateTimeOffset(2025, 2, 12, 14, 0, 0, TimeSpan.Zero),
                StepHash = "HASH",
            },
            new TestSagaState(),
            "saga",
