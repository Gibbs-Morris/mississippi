...
public sealed class CosmosReplicaSinkDeliveryStateStoreTests
{
    private sealed class FakeShard : ICosmosReplicaSinkShard
    {
        ...
        public Task<IReadOnlyList<ReplicaSinkDeliveryState>> ReadDeadLettersAsync(
            ...
            Task.FromResult<IReadOnlyList<ReplicaSinkDeliveryState>>(
                StoredStates.Values.Where(static state => state.DeadLetter is not null)
        ...
        public Task<IReadOnlyList<ReplicaSinkDeliveryState>> ReadDueRetriesAsync(
            ...
            Task.FromResult<IReadOnlyList<ReplicaSinkDeliveryState>>(
                StoredStates.Values
                    .Where(state =>
                        state.Retry?.NextRetryAtUtc is not null &&
                        (state.Retry.NextRetryAtUtc.Value <= dueAtOrBeforeUtc))
    ...
    public async Task CosmosReplicaSinkDeliveryStateStoreShouldMergeDueRetriesAcrossShards()
    {
        ...
        CosmosReplicaSinkDeliveryStateStore store = new(
            [east, west],
            NullLogger<CosmosReplicaSinkDeliveryStateStore>.Instance);
    ...
    public async Task CosmosReplicaSinkDeliveryStateStoreShouldPageDeadLettersAcrossShards()
    {
        ...
        east.Seed(
            new ReplicaSinkDeliveryState(
                "Projection::east::orders-read::1",
                10,
                deadLetter: new(10, 1, "dead-east-1", "Dead east 1", new(2026, 3, 30, 12, 1, 0, TimeSpan.Zero))),
            new ReplicaSinkDeliveryState(
                "Projection::east::orders-read::2",
                11,
                deadLetter: new(11, 1, "dead-east-2", "Dead east 2", new(2026, 3, 30, 12, 3, 0, TimeSpan.Zero))));
        west.Seed(
            new ReplicaSinkDeliveryState(
                "Projection::west::orders-read::1",
                12,
                deadLetter: new(12, 1, "dead-west-1", "Dead west 1", new(2026, 3, 30, 12, 2, 0, TimeSpan.Zero))));
        CosmosReplicaSinkDeliveryStateStore store = new(
            [east, west],
            NullLogger<CosmosReplicaSinkDeliveryStateStore>.Instance);
    ...
    public async Task CosmosReplicaSinkDeliveryStateStoreShouldRouteStateReadsAndWritesBySinkKey()
    {
        ...
        ReplicaSinkDeliveryState? roundTrippedEast = await store.ReadAsync(
            eastState.DeliveryKey,
            CancellationToken.None);
        ReplicaSinkDeliveryState? roundTrippedWest = await store.ReadAsync(
            westState.DeliveryKey,
            CancellationToken.None);
