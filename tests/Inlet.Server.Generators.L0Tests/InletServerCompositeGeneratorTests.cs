...
/// <summary>
///     Tests for <see cref="InletServerCompositeGenerator" />.
/// </summary>
public sealed class InletServerCompositeGeneratorTests
{
    /// <summary>
        ...
        )
    {
        ...
        InletServerCompositeGenerator generator = new();
    ...
    /// <summary>
    ///     Generated composite registration should include server entry points and layered registrations.
    /// </summary>
    [Fact]
    public void GeneratedCompositeIncludesServerEntryPoints()
    {
        const string compositeSource = """
                                       using Mississippi.Inlet.Generators.Abstractions;
                                       using Mississippi.Inlet.Abstractions;

                                       [assembly: GenerateInletServerComposite(AppName = "TestApp")]
        ...
        SyntaxTree tree = runResult.GeneratedTrees.Single(t => t.FilePath.EndsWith(
            "TestAppServerRegistrations.g.cs",
            StringComparison.Ordinal));
        string generatedCode = tree.GetText().ToString();
        Assert.Contains(
            "public static MississippiServerBuilder AddTestAppServer",
            generatedCode,
            StringComparison.Ordinal);
        Assert.Contains("public static WebApplication UseTestAppServer", generatedCode, StringComparison.Ordinal);
        Assert.Contains(
            "builder.HostBuilder.AddTestAppServerObservability();",
            generatedCode,
            StringComparison.Ordinal);
        Assert.Contains("builder.HostBuilder.AddTestAppOrleansClient();", generatedCode, StringComparison.Ordinal);
        Assert.Contains("builder.Services.AddTestAppApi();", generatedCode, StringComparison.Ordinal);
        Assert.Contains("builder.Services.AddTestAppRealtime();", generatedCode, StringComparison.Ordinal);
        Assert.Contains("app.UseTestAppMiddleware();", generatedCode, StringComparison.Ordinal);
        Assert.Contains("app.MapTestAppEndpoints();", generatedCode, StringComparison.Ordinal);
    }
}