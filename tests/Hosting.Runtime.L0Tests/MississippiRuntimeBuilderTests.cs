...
public sealed class MississippiRuntimeBuilderTests
{
    ...
    private static WebApplicationBuilder CreateBuilder() => CreateBuilder(Environments.Development);
    ...
    public void AddMississippiRuntimeAllowsExplicitlyApprovedLoopbackOutsideDevelopment()
    {
        ...
        builder.Configuration["ConnectionStrings:cosmos"] =
            $"AccountEndpoint=http://localhost:8081/;AccountKey={SecretValue};";
    ...
    public void AddMississippiRuntimeAllowsLoopbackConnectionStringsInDevelopment()
    {
        ...
        builder.Configuration["ConnectionStrings:cosmos"] =
            $"AccountEndpoint=http://{IPAddress.Loopback}:8081/;AccountKey={SecretValue};";
        MississippiRuntimeBuilder runtimeBuilder = builder.AddMississippiRuntime();
        Assert.NotNull(runtimeBuilder);
    }
    ...
    public void AddMississippiRuntimeRejectsConflictingEndpointSet()
    {
        ...
        Assert.Contains(
            "mixes local or emulator hosts with external hosts",
            exception.Message,
    ...
    public void AddMississippiRuntimeRejectsDuplicateAttachment()
    {
        ...
        InvalidOperationException exception =
            Assert.Throws<InvalidOperationException>(() => builder.AddMississippiRuntime());
        Assert.Contains(
            "AddMississippiRuntime can only be called once per host",
            exception.Message,
            StringComparison.Ordinal);
    }
    ...
    public void AddMississippiRuntimeRejectsInsecureExternalEndpointTransport()
    {
        WebApplicationBuilder builder = CreateBuilder(Environments.Production);
        builder.Configuration["ConnectionStrings:cosmos"] =
            $"AccountEndpoint=http://prod.example.com/;AccountKey={SecretValue};";
        InvalidOperationException exception =
            Assert.Throws<InvalidOperationException>(() => builder.AddMississippiRuntime());
        Assert.Contains("[config-trust]", exception.Message, StringComparison.Ordinal);
        Assert.Contains("scheme 'http'", exception.Message, StringComparison.Ordinal);
    ...
    public void AddMississippiRuntimeRejectsLoopbackConnectionStringsOutsideDevelopment()
    {
        WebApplicationBuilder builder = CreateBuilder(Environments.Production);
        builder.Configuration["ConnectionStrings:cosmos"] =
            $"AccountEndpoint=http://127.0.0.1:8081/;AccountKey={SecretValue};";
        InvalidOperationException exception =
            Assert.Throws<InvalidOperationException>(() => builder.AddMississippiRuntime());
        Assert.Contains("[config-trust]", exception.Message, StringComparison.Ordinal);
        Assert.Contains("ConnectionStrings:cosmos", exception.Message, StringComparison.Ordinal);
    ...
    public void ApplyOrleansConfigurationAllowsAdditiveSiloTuningThatPreservesOwnership()
    {
        WebApplicationBuilder builder = CreateBuilder();
        MississippiRuntimeBuilder runtimeBuilder = builder.AddMississippiRuntime();
        runtimeBuilder.Orleans(silo =>
            silo.Configure<SiloOptions>(options => options.SiloName = "allowed-runtime-silo"));
        ...
        Assert.Contains(
            ...
            descriptor => descriptor.ServiceType.FullName?.Contains(nameof(SiloOptions), StringComparison.Ordinal) ==
                          true);
    }

    /// <summary>
    ///     Runtime-owned Orleans attachment should reject clustering ownership overrides introduced through queued callback
    ///     execution.
    /// </summary>
    [Fact]
    public void ApplyOrleansConfigurationRejectsClusteringOwnershipOverrideAddedThroughQueuedCallback()
    {
        ...
        MississippiRuntimeBuilderState state = GetState(builder.Services);
        InvalidOperationException exception =
            Assert.Throws<InvalidOperationException>(() => state.ApplyOrleansConfiguration(new TestSiloBuilder()));
        AssertFrozenOwnershipViolation(exception, "clustering");
    }
    ...
    /// <summary>
    ///     Runtime-owned Orleans attachment should reject competing Orleans ownership introduced through queued callback
    ///     execution.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    ///     Runtime-owned Orleans attachment should reject endpoint ownership overrides introduced through queued callback
    ///     execution.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    ///     Runtime-owned Orleans attachment should reject provider ownership overrides introduced through queued callback
    ///     execution.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    ///     Runtime-owned Orleans attachment should reject storage ownership overrides introduced through queued callback
    ///     execution.
    /// </summary>
    [Fact]
